...
Retiring a skill now leaves a tombstone. `dev-chronicle` is listed in `packages/cli/retired.json`, so a group update on a machine that installed it removes the installer-owned copy and names its replacement, a locally edited copy is kept and reported as `--force` handles elsewhere, and `skills remove dev-chronicle` still works by name while installing one refuses. Without it an upgraded machine kept the old skill triggering beside dev-status.
