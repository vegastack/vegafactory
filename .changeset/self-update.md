...
- npm is asked at most once an hour and the attempt is remembered in `~/.vegafactory/update.json` — the attempt, not just a successful answer, so a registry that is down costs one call an hour rather than one every pass, and an install that failed is not retried until the next hour either. An idle worker polls every couple of minutes; asking each pass was hundreds of calls a day for a version that changes weekly.
