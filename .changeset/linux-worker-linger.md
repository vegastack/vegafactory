...
- The systemd unit writes `worker.log` and `worker.err.log`, the same two files the macOS one does. The path was already being passed to the Linux branch and dropped, so the log this product tells people to read never appeared there. The journal still has everything as well.
