...
- An interrupted global skill install moves with the rest: its journal and its lock. Left behind, the journal would never be recovered and the next add would roll its backups forward, bringing back skills somebody had removed.
- A symlinked home on either side refuses rather than moving state somewhere neither path names, and a cross-device home falls back to copy-then-remove instead of failing on `EXDEV`.
