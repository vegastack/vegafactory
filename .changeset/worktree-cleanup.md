...
- A second run on an issue resumes the agent thread the first one started, and forks it once the branch has moved. Claude Code's `--session-id` is derived from the repository and the issue rather than remembered, so a resume needs nothing recorded; what is recorded is the commit the thread last saw. The head is recorded again when the run ends, so the agent's own commit is not read as the branch moving under it. A rebase, a review round that rewrote the work, or a commit from another machine is, and the next run is then a new thread. `codex exec` takes no session id, so none is passed and nothing claims otherwise.
