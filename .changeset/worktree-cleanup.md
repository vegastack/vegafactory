...
- A worktree that was touched recently is not idle, whatever its last commit and its ledger say. Nothing outside the worker registers a session anywhere, so an attended run in another terminal is invisible to the pass — its files are not, and that is the difference between "nobody has committed" and "nobody is working".
