...
test('an old export proof cannot survive an approved-base rebind',async()=>{
  const f=await fixture(async(cwd,g)=>{await writeFile(join(cwd,'allowed.txt'),'first');g('add','.');g('commit','-m','first progress')})
  try{
    const controller={root:f.root,verifyAuthority:async()=>{}},candidate=await prepareCheckpoint({run:f.run,approvedIntent:f.intent,headSha:f.head},controller)
    const reboundIntent={...f.intent,baseSha:f.head},rebound=parseRun({...f.run,baseSha:f.head,checkpointIntent:reboundIntent})
    await atomicRunFile(join(f.root,f.run.runId,'run.json'),rebound)
    const result=await publishCheckpoint(candidate,controller)
    expect(result).toMatchObject({kind:'refused',reason:'checkpoint-intent-refused'});expect(f.g('ls-remote','origin','refs/heads/feat/1-work')).toBe('')
  }finally{await rm(f.home,{recursive:true,force:true})}
},15000)

test('checkpoint push runs the exact pre-admission pre-push hook snapshot',async()=>{
  const f=await fixture(async(cwd,g)=>{await writeFile(join(cwd,'allowed.txt'),'changed');g('add','.');g('commit','-m','safe progress')},{prePush:home=>`#!/bin/sh\nprintf ran > ${JSON.stringify(join(home,'pre-push-ran'))}\n`})
  try{
    const candidate=await prepareCheckpoint({run:f.run,approvedIntent:f.intent,headSha:f.head},{root:f.root,verifyAuthority:async()=>{}}),result=await publishCheckpoint(candidate,{root:f.root,verifyAuthority:async()=>{}})
    expect(result.kind).toBe('acknowledged');expect(await Bun.file(join(f.home,'pre-push-ran')).text()).toBe('ran')
  }finally{await rm(f.home,{recursive:true,force:true})}
},15000)

test('a hook added after the no-hook admission snapshot never executes',async()=>{
  const f=await fixture(async(cwd,g)=>{await writeFile(join(cwd,'allowed.txt'),'changed');g('add','.');g('commit','-m','safe progress')})
  const marker=join(f.home,'late-hook-ran'),hook=join(f.cwd,'.git','hooks','pre-push')
  try{
    await writeFile(hook,`#!/bin/sh\nprintf ran > ${JSON.stringify(marker)}\n`);await chmod(hook,0o755)
    const candidate=await prepareCheckpoint({run:f.run,approvedIntent:f.intent,headSha:f.head},{root:f.root,verifyAuthority:async()=>{}}),result=await publishCheckpoint(candidate,{root:f.root,verifyAuthority:async()=>{}})
    expect(result.kind).toBe('acknowledged');expect(await Bun.file(marker).exists()).toBe(false)
  }finally{await rm(f.home,{recursive:true,force:true})}
},15000)
