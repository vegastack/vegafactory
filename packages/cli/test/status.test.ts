...
test('138 CLI group-recovery status survives the dashboard bridge and reaches operator attention',async()=>{
  ...
  const shared={head,refusal:null,history:{coverage:'complete',archiveCoverage:'partial',sourceCommit:head},tasks:[{taskKey,repo:'acme/app',issue:144,state:'recovery-queued',machineId:'receiver',generation:2,sourceCommit:head,originMachineId:'original',lastTransitionObservedAt:'2026-09-11T00:00:00Z',checkpoint:null,history:{coverage:'complete',events:[{kind:'group-succession',generation:2,machineId:'receiver',previousMachineId:'original',sourceCommit:head,observedAt:'2026-09-11T00:00:00Z'}]}}]}
  const recovery=[{taskKey,issue:144,state:'recovery-queued',action:'wait' as const,reason:'parent must start before this recovered child'}]
  const emitted=buildStatus({config,state:{lastTick:{},handled:[]},lockPid:null,repos:[{repo:'acme/app',policy,board:[],worktrees:[],logs:[],shared:shared as never,recovery}]})
  const source=JSON.parse(JSON.stringify(emitted))
