...
test.each(['disjoint', 'absent', 'prefix-overlap', 'equal-resource', 'same-issue', 'selected-mismatch', 'invalid-peer', 'authority-drift'] as const)('default shared adapters admit the %s two-home fleet projection', async scenario => {
    ...
    try {
        ...
        let head = stateRoot, sequence = 1, activeVendor = 0, maxVendor = 0, drift = false, admissionWaiters: Array<() => void> = [];
