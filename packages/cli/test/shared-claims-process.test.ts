...
import { canonical } from '../src/shared-claims.ts';
for (const independent of [false, true])
    test(`actual separate-home claimants: ${independent ? 'independent tasks both enter' : 'one task enters once'}`, async () => {
...
const [url,rootDir,machineId,index,independent]=process.argv.slice(2);const localRoot=rootDir+'/'+machineId;await mkdir(localRoot,{mode:0o700});
...
const target={host:'github.com',repository:'acme/control',repositoryId:'R_state',branch:'factory-state',rootCommit:'1'.repeat(40),installationId:${JSON.stringify(installation)},localRoot,provider,verifyCandidate:async()=>{},verifyTransition:async()=>{},verifyEvidence:async()=>{}};
...
const result=await acquireSharedTask({machine,session,candidate,operationId:randomUUID()});if(result.kind==='owned')await appendFile(rootDir+'/sentinel',machineId+'\\n');process.stdout.write(JSON.stringify(result.kind==='owned'?{kind:result.kind}:{kind:result.kind,reason:result.reason}));`);
        try {
            const workers = [0, 1].map(i => Bun.spawn([process.execPath, worker, server.url.toString(), dir, 'machine-' + i, String(i), String(independent)], { stdout: 'pipe', stderr: 'pipe' }));
