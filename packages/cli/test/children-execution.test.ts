...
test('real later child handoff preserves accepted join history and schedules only unfinished work',async()=>{
  ...
  try{
    ...
    checkpointFlushSpy=spyOn(checkpointOwner,'flushRunCheckpoint').mockImplementation(async run=>{const expected=f.material.children.find(row=>row.task.runId===run.runId)?.checkpointRequest??undefined;expect(run.checkpointIntent?.approvalRequest).toEqual(expected)})
...
test('stopped-group restart starts the parent before only unfinished recovery-queued children',async()=>{
 ...
 const bad=child(unfinishedClaim) as any;bad.state='running';calls.length=0
