...
async function realProgressedSuccessionFixture(options:{stoppedBeforeRecovery?:boolean;lostGroupReadback?:boolean;driftAfterGroup?:boolean;statusClaimedAfterRecovery?:boolean;sameHomeTask?:'parent'|'unfinished-child'}={}){
  const owner=await import('../src/shared-claims.ts'),{processIdentity}=await import('../src/claims.ts'),{readHostBinding}=await import('../src/machine-identity.ts'),home=await realpath(await mkdtemp(join(tmpdir(),'child-succession-real-'))),repo=join(home,'repo'),digest='d'.repeat(64),installation=randomUUID()
  const checks=join(home,'checks'),vendors=join(home,'vendors')
  const control=options.stoppedBeforeRecovery?'control-room: acme/control#dev\nsync-max-age: 2h\ndispatch: local\n':''
  await mkdir(join(repo,'.vegastack'),{recursive:true});await writeFile(join(repo,'.vegastack','dev.md'),`repo: acme/app · default branch main\n${control}operators: robot\ncommands: check \`echo check >> ${checks}\`\nimplement: codex fixture high\n`);await writeFile(join(repo,'.gitignore'),'.vegastack/.worktrees/\n');await writeFile(join(repo,'base.txt'),'base\n');git(repo,'init','-b','task/1');git(repo,'config','user.name','Fixture');git(repo,'config','user.email','fixture@example.invalid');git(repo,'add','.');git(repo,'commit','-m','base');git(repo,'remote','add','origin','https://github.com/acme/app.git');const rootSha=git(repo,'rev-parse','HEAD'),rootTree=git(repo,'rev-parse','HEAD^{tree}')
  let head=rootSha,sequence=1,failBranchReads=0,groupMutations=0,faultParentTaskKey:string|null=null,childCompleteReadFailures=0,failBeforeParentStop=false,historyMode:'normal'|'missing-group'|'repeated-group'='normal';const versions=new Map([[head,{'coordination/index.json':owner.canonical({schemaVersion:1,installationId:installation,revision:0,active:[],machines:[]})} as Record<string,string>]]),order=[head]
  const target:import('../src/shared-claims.ts').CoordinationTarget={host:'github.com',repository:'acme/control',repositoryId:'R_state',branch:'factory-state',rootCommit:rootSha,installationId:installation,localRoot:join(home,'coordination'),provider:{branch:async()=>{if(failBranchReads>0){failBranchReads--;throw Error('controlled lost group readback')}return{id:'REF',head,repositoryId:'R_state',private:true,defaultBranch:'main'}},read:async(_t,at,path)=>versions.get(at)?.[path]??null,compare:async(_t,base,next)=>base===next?'identical':order.indexOf(base)>=0&&order.indexOf(base)<order.indexOf(next)?'ahead':'diverged',commit:async(_t,input)=>{if(input.expectedHeadOid!==head)return{kind:'conflict',reason:'changed'};const bodies=Object.values(input.files),group=bodies.some(body=>body.includes('"type":"group-succession"')),receipts=bodies.flatMap(body=>{try{const row=JSON.parse(body);return row?.schemaVersion===1&&typeof row.type==='string'&&typeof row.taskKey==='string'?[row]:[]}catch{return[]}}),childComplete=receipts.some(row=>row.type==='complete'&&row.taskKey!==faultParentTaskKey),parentStop=receipts.some(row=>row.type==='stop'&&row.taskKey===faultParentTaskKey);if(parentStop&&failBeforeParentStop){failBeforeParentStop=false;throw Error('controlled crash before parent stop commit')}head=(++sequence).toString(16).padStart(40,'0');versions.set(head,{...versions.get(input.expectedHeadOid),...input.files});order.push(head);if(childComplete&&childCompleteReadFailures){failBranchReads=childCompleteReadFailures;childCompleteReadFailures=0;return{kind:'ambiguous' as const,reason:'controlled crash after accepted child completion'}}if(group){groupMutations++;if(options.driftAfterGroup)approvalComment.body+='\npost-CAS authority drift';if(options.lostGroupReadback){failBranchReads=2;return{kind:'ambiguous' as const,reason:'controlled lost response'}}}return{kind:'committed' as const,head}}},verifyCandidate:async()=>{},verifyTransition:async()=>{},verifyEvidence:async()=>{},verifyChildRelationship:async()=>({maxChildren:2}),verifyGroupSuccession:async()=>({maxChildren:2}),verifySession:async()=>{},random:()=>0}
  target.provider.history=async(_target,current)=>{const start=order.indexOf(current);let commits=order.slice(0,start+1).reverse().map((oid,index)=>{const at=order.indexOf(oid),previous=at>0?versions.get(order[at-1]!):undefined,operation=Object.keys(versions.get(oid)??{}).find(path=>path.startsWith('coordination/operations/')&&!Object.hasOwn(previous??{},path))?.split('/').at(-1)?.replace('.json','');return{oid,parents:at>0?[order[at-1]!]:[],headline:operation?'factory coordination '+operation:'fixture root',committedAt:new Date(Date.now()-index).toISOString()}});const headline='factory coordination '+request.operationId,index=commits.findIndex(row=>row.headline===headline);if(historyMode==='missing-group')commits=commits.filter(row=>row.headline!==headline);if(historyMode==='repeated-group'&&index>=0)commits.splice(index+1,0,{...commits[index]!});return{commits,nextCursor:null}}
  const machine={id:'original',installationId:randomUUID(),hostBindingDigest:options.sameHomeTask?(await readHostBinding()).digest:'a'.repeat(64),executionLogin:'robot',group:'dev',enabled:true,allowedRepositories:['acme/app'],repositoryIds:{'acme/app':'R_app'},policyDigest:digest,coordination:{repositoryId:'R_state',repository:'acme/control',branch:'factory-state',rootCommit:rootSha,installationId:installation},defaults:{maxRuns:1,childConcurrent:2,recovery:'verified-transfer' as const}},session={target,localRoot:join(home,'original'),machineId:machine.id,installationId:machine.installationId,sessionId:randomUUID(),hostBindingDigest:machine.hostBindingDigest,bootIdDigest:digest,identity:await processIdentity()}
  const approval=await import('../../../skills/dev/dev-implement/scripts/lib/approval.mjs'),parentBody='<!-- vsk:v1 type=brief rev=1 scope=quick-build -->\n## Outcome\nRecover children.\n',parentPlan='<!-- vsk:v1 type=plan rev=1 -->\n- [ ] **Task 1: parent** <!-- task-id:1-T1 -->\n  - Files — `src/child-0`, `src/child-1`\n  - Interfaces — children\n  - Steps: recover\n\n**Independent groups:**\n- `a` — #8 · Files: `src/child-0`\n- `b` — #9 · Files: `src/child-1`\n',issueRows=new Map<number,any>(),commentRows=new Map<number,any[]>();let sameHomeCheckout:string|null=null,acceptedReviewAt:string|null=null,acceptedReviewSuffix='',dependencyRows:any[]=[]
  ...
  const gh=async(args:string[])=>{const endpoint=(args.find(arg=>arg==='user'||arg==='graphql'||arg.startsWith('repos/'))??'').split('?')[0]!;let value:unknown;if(endpoint==='user')value={login:'robot'};else if(endpoint==='graphql')value={data:{node:{id:'I_1',number:1,repository:{id:'R_app',nameWithOwner:'acme/app'}}}};else if(endpoint==='repos/acme/app')value={node_id:'R_app',full_name:'acme/app',private:true,default_branch:'main'};else if(endpoint==='repos/acme/app/issues/comments/12')value=approvalComment;else if(endpoint.endsWith('/dependencies/blocked_by'))value=dependencyRows;else{const history=/issues\/(\d+)\/comments$/.exec(endpoint),subject=/issues\/(\d+)$/.exec(endpoint);if(history){const issue=Number(history[1]),rows=commentRows.get(issue);value=rows;let executed=false;try{executed=(await readFile(vendors,'utf8')).includes('vendor')}catch{}if(issue===9&&sameHomeCheckout&&executed){const plan=rows![0]!,head=git(sameHomeCheckout,'rev-parse','HEAD'),scope=childArtifacts[1]!.find(row=>row.kind==='plan')!.digest;acceptedReviewAt??=new Date().toISOString();value=[{...plan,body:plan.body.replace('- [ ]','- [x]')},{id:92,node_id:'REVIEW_9',user:{login:'robot'},updated_at:acceptedReviewAt,issue_url:'https://api.github.com/repos/acme/app/issues/9',body:`<!-- vsk:v1 type=review sha=${head} verdict=clean agent=codex -->\n\`\`\`json\n${JSON.stringify({reviewBinding:{sha:head,baseSha:rootSha,scopeDigest:scope,verdict:'clean',findings:[]}})}\n\`\`\`\n${acceptedReviewSuffix}`}]}}else if(subject)value=issueRows.get(Number(subject[1]));else throw Error('unexpected recovery endpoint '+endpoint)}if(value===undefined)throw Error('missing recovery endpoint '+endpoint);return (args.includes('--include')?'HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n\r\n':'')+JSON.stringify(value)}
  ...
  for(const [candidate,claim] of [[parentCandidate,parentStarted.claim],...childCandidates.map((candidate,index)=>[candidate,childClaims[index]!] as const)] as const){const qualified=await owner.publishRecoveryReceipt({claim,operationId:randomUUID(),payload:{schemaVersion:2,kind:'execution-qualification',harness:'codex',harnessVersion:'fixture',model:'fixture',effort:'high',accountRef:'account',configurationDigest:digest,candidateSha:rootSha,validationIds:[candidate.approvedTaskIds[0]+'/check/'+digest],managedKinds:['checkpoint-push','handback','evidence','telemetry-push'],unmanagedDenied:true,result:'qualified'}}),branch=candidate.issue===1?'task/1':childCheckpoints.find(row=>row.child.issue===candidate.issue)!.child.branch,checkpoint={schemaVersion:1 as const,id:randomUUID(),repo:'acme/app',repositoryId:'R_app',branch,baseSha:rootSha,headSha:rootSha,treeSha:rootTree,scopeDigest:candidate.scopeDigest,runId:claim.runId,publishedAt:new Date().toISOString()},recovery={schemaVersion:2 as const,taskKey:claim.taskKey,runId:claim.runId,generation:claim.generation,approvalBindings:[authority],recordBinding:authority,scopeDigest:candidate.scopeDigest,approvalDigest,execution:{providerMode:'subscription' as const,harness:'codex' as const,harnessVersion:'fixture',model:'fixture',effort:'high',accountRef:'account',qualification:qualified.reference},checkpoint,completed:[],children:[],joins:[],effects:[],remoteEffectCoverage:{kind:'qualified-managed-only' as const,qualification:qualified.reference}};const saved=await owner.transitionSharedTask({claim,operationId:randomUUID(),transition:{kind:'checkpoint',checkpoint,recovery}});if(saved.kind!=='owned')throw Error(saved.reason);prepared.push({candidate,claim:saved.claim,checkpoint,recovery})}
  ...
  const acceptedScope={schemaVersion:2 as const,repo:'acme/app',issue:8,artifacts:childArtifacts[0]!,approvalBindings:[authority],approvedTaskIds:['8-T1'],completedTaskIds:['8-T1'],parentRepo:'acme/app',parentIssue:1,parentBefore:rootSha,parentAfter:rootSha,acceptedAt:new Date().toISOString()},scopeReceipt=await owner.publishRecoveryReceipt({claim:childA.claim,operationId:randomUUID(),payload:{...acceptancePayload,acceptedScope}}),scopeLinked=await owner.linkAcceptedScope({claim:scopeReceipt.claim,operationId:randomUUID(),acceptedScope:scopeReceipt.reference});if(scopeLinked.kind!=='owned')throw Error(scopeLinked.reason);childA.claim=scopeLinked.claim
  ...
  const receiver={...machine,id:options.sameHomeTask?machine.id:'receiver',installationId:options.sameHomeTask?machine.installationId:randomUUID(),hostBindingDigest:(await readHostBinding()).digest,defaults:{...machine.defaults,childConcurrent:2}},receiverSession={...session,machineId:receiver.id,installationId:receiver.installationId,sessionId:randomUUID(),hostBindingDigest:receiver.hostBindingDigest,localRoot:join(home,'receiver')},before=await owner.readCoordination(target),request={schemaVersion:1 as const,kind:'recover-stopped-group' as const,operationId:randomUUID(),expectedHead:before.head,parentTaskKey:parentStarted.claim.taskKey,groupPlan:{repo:'acme/app',issue:1,kind:'plan' as const,artifactId:'PLAN_1',rev:1,digest:parentArtifacts.find(row=>row.kind==='plan')!.digest},groupsDigest:'e'.repeat(64),members:prepared.map(row=>({expected:{taskKey:row.claim.taskKey,runId:row.claim.runId,generation:row.claim.generation,ownerToken:row.claim.ownerToken,machineId:row.claim.machineId,installationId:row.claim.installationId,sessionId:row.claim.sessionId},candidate:row.candidate})).sort((a,b)=>a.expected.taskKey.localeCompare(b.expected.taskKey))}
  const dispatch=await import('../src/dispatch.ts')
  const operations=()=>order.slice(1).flatMap((at,index)=>{const previous=versions.get(order[index]!)??{},files=versions.get(at)??{},path=Object.keys(files).find(key=>key.startsWith('coordination/operations/')&&!Object.hasOwn(previous,key));if(!path)return[];const receipt=JSON.parse(files[path]!);return[{at,type:receipt.type,taskKey:receipt.taskKey??receipt.parentTaskKey,operationId:receipt.operationId}]})
  if(options.stoppedBeforeRecovery){
   let sameHomeRunId:string|null=null
   if(options.sameHomeTask){
    const runtime=await import('../src/runs.ts'),selected=options.sameHomeTask==='parent'?prepared[0]!:prepared[2]!,artifacts=selected.candidate.issue===1?parentArtifacts:childArtifacts[selected.candidate.issue===8?0:1]!,branch=selected.checkpoint.branch
    let checkout=repo
    if(selected.candidate.issue!==1){const planner=await import('../../../skills/dev/dev-implement/scripts/children.mjs'),planned=planner.planParallelRun({groups:[{id:'a',members:['#8'],files:['src/child-0']},{id:'b',members:['#9'],files:['src/child-1']}],issues:{8:{number:8,title:'child 8',type:'feat'},9:{number:9,title:'child 9',type:'feat'}},parentBranch:'task/1',parentHead:rootSha,repoRoot:repo,parentIssue:1}).children.find(row=>row.issue===selected.candidate.issue);if(!planned||planned.branch!==branch)throw Error('same-home fixture child plan differs');git(repo,'branch',branch,rootSha);checkout=planned.path;sameHomeCheckout=checkout;await mkdir(join(checkout,'..'),{recursive:true});git(repo,'worktree','add',checkout,branch)}
    const approvalBinding={commentId:12,bodySha256:owner.sha256(approvalBody)},authorityRequest={kind:'consolidated' as const,parentRepo:'acme/app',parentIssue:1,approvalBinding,requested:{repo:'acme/app',issue:selected.candidate.issue,taskIds:selected.candidate.approvedTaskIds,actionId:'local',branch:'task/1',baseSha:rootSha,paths:selected.candidate.paths.length?selected.candidate.paths:['src/child-0','src/child-1'],operation:'edit' as const}},childAction=childCheckpoints.find(row=>row.child.issue===selected.candidate.issue),checkpointRequest=childAction?{parentRepo:'acme/app',parentIssue:1,approvalBinding,requested:{repo:'acme/app',issue:selected.candidate.issue,taskIds:selected.candidate.approvedTaskIds,actionId:childAction.id,branch:childAction.child.branch,ref:childAction.child.ref,baseSha:rootSha,paths:childAction.child.paths,operation:'checkpoint' as const}}:null
    const checkpointIntent=checkpointRequest?{id:checkpointRequest.requested.actionId,repo:'acme/app',repositoryId:'R_app',remote:'origin',remoteUrl:'https://github.com/acme/app.git',branch,baseRef:checkpointRequest.requested.ref,baseSha:rootSha,scopeDigest:selected.candidate.scopeDigest,paths:selected.candidate.paths,approvalBindings:[authority],approvalRequest:checkpointRequest}:undefined
    const created=await runtime.createRun({root:runsRoot(home),runId:selected.candidate.runId,repo:'acme/app',issue:selected.candidate.issue,parent:selected.candidate.issue===1?null:1,checkout:await realpath(checkout),branch,baseSha:rootSha,headSha:rootSha,stage:'implement',harness:selected.recovery.execution.harness,model:selected.recovery.execution.model,effort:selected.recovery.execution.effort,execution:selected.recovery.execution,runtimeBinding:{schemaVersion:1,sourceSha:rootSha,treeSha:rootTree,packageName:'@vegastack/vegafactory',version:'1.0.0',tarballSha256:'6'.repeat(64),inventoryDigest:'7'.repeat(64)},configurationDigest:digest,approvalBindings:[authority],recordBinding:authority,approvalRefs:artifacts,policyDigest:digest,claimToken:randomUUID(),startedAt:new Date().toISOString(),taskKey:{repo:'acme/app',issue:selected.candidate.issue,taskId:selected.candidate.approvedTaskIds[0]!,scopeDigest:selected.candidate.scopeDigest},approvedTaskIds:selected.candidate.approvedTaskIds,activeElapsedMs:null,taskOwner:null,agentAccountOwner:null,accountRef:'account',waitReason:null,hostBindingDigest:machine.hostBindingDigest,machine:{id:machine.id,installationId:machine.installationId,sessionId:session.sessionId,hostBindingDigest:machine.hostBindingDigest},sharedClaim:{taskKey:selected.claim.taskKey,generation:selected.claim.generation,ownerToken:selected.claim.ownerToken,stateCommit:selected.claim.stateCommit},checkpoint:selected.checkpoint,...(checkpointIntent?{checkpointIntent}:{}),remoteEffectCoverage:selected.recovery.remoteEffectCoverage,authorityRequest,handbackIntent:{id:'run-handback',approvalBindings:[authority]},dispatchRequest:{commentId:null,reactionId:null},worktreeDigest:await runtime.worktreeFingerprint(checkout)})
    await runtime.transitionRun(created.runId,created.generation,{state:'terminal',terminationCause:'interrupted',finishedAt:new Date().toISOString()},runsRoot(home));sameHomeRunId=created.runId
   }
   const qualification=prepared[0]!.recovery.execution.qualification,mutateEvidenceReceipt=()=>{const path=owner.operationPath(qualification.operationId),files=versions.get(qualification.commitSha)!,body=files[path]!;files[path]=owner.canonical({...JSON.parse(body),requestDigest:'f'.repeat(64)});return()=>{files[path]=body}}
   return{home,repo,checks,vendors,owner,target,gh,config,dispatch,parentTaskKey:parentStarted.claim.taskKey,receiver,request,parentRecovery,rootSha,rootTree,sameHomeRunId,oldClaims:prepared.map(row=>row.claim),get stateHead(){return head},get groupMutations(){return groupMutations},operations,setDependencies:(rows:any[])=>{dependencyRows=rows},mutateEvidenceReceipt,mutateApproval:()=>{const body=approvalComment.body;approvalComment.body+='\ncontrolled authority drift';return()=>{approvalComment.body=body}},mutateAcceptedReview:()=>{acceptedReviewSuffix='Stop before parent launch.';return()=>{acceptedReviewSuffix=''}},crashAfterNextChildComplete:()=>{childCompleteReadFailures=20},blockAfterNextChildComplete:()=>{childCompleteReadFailures=1},crashBeforeNextParentStop:()=>{failBeforeParentStop=true},resumeReadback:()=>{failBranchReads=0},cleanup:()=>rm(home,{recursive:true,force:true})}
  }
  const recovered=await owner.recoverStoppedGroup({machine:receiver,session:receiverSession,request});if(recovered.kind!=='owned')throw Error(recovered.reason)
  ...
  const mutateGroupReceipt=()=>{const path=owner.operationPath(request.operationId),files=versions.get(head)!,before=files[path]!;files[path]=owner.canonical({...JSON.parse(before),operationId:randomUUID()});return()=>{files[path]=before}}
  if(options.statusClaimedAfterRecovery)return{home,repo,target,gh,config,parent:recovered.parent,children:recovered.children,request,get stateHead(){return head},mutations:()=>order.length,setHistoryMode:(mode:typeof historyMode)=>{historyMode=mode},mutateCurrentTask,mutateGroupReceipt,cleanup:()=>rm(home,{recursive:true,force:true})}
  const material=await dispatch.inspectRemoteRecovery({repo:'acme/app',taskKey:recovered.parent.taskKey,config},{target,gh,source:{repository:async()=>({node_id:'R_app'}),fetch:async()=>{}}});if(material.blocks.length)throw Error('fixture recovery blocked: '+material.blocks.join('; '));dispatch.assertRemoteRecoveryMaterial(material)
  ...
  return{home,repo,checks,vendors,owner,target,gh,config,dispatch,material,runningParent:runningParent.claim,laterChild:handoff.claim,unfinishedChild:recovered.children.find(row=>row.runId===childClaims[1]!.runId)!,inspection,parentRecovery,acceptedRunId:childA.claim.runId,unfinishedRunId:childClaims[1]!.runId,rootSha,rootTree,receiver,later,childArtifacts,mutateCurrentTask,get stateHead(){return head},mutations:()=>order.length,cleanup:()=>rm(home,{recursive:true,force:true})}
}
...
test('real later child handoff preserves accepted join history and schedules only unfinished work',async()=>{
  const f:any=await realProgressedSuccessionFixture(),{spyOn}=await import('bun:test'),gateway=await import('../src/children.ts'),runtime=await import('../src/runs.ts'),ghOwner=await import('../src/gh.ts'),{loadConfiguredPolicy}=await import('../src/control-room.ts')
  const previousPath=process.env.PATH
  let targetSpy:ReturnType<typeof spyOn>|null=null,claimSpy:ReturnType<typeof spyOn>|null=null,ghSpy:ReturnType<typeof spyOn>|null=null,checkpointSpy:ReturnType<typeof spyOn>|null=null,checkpointFlushSpy:ReturnType<typeof spyOn>|null=null
  try{
    expect(f.inspection.kind).toBe('verified')
    if(f.inspection.kind!=='verified')throw Error(f.inspection.reason)
    const progressed=f.inspection.currentMembers.find((row:any)=>row.initial.runId===f.acceptedRunId)!
    ...
    const currentByRun=new Map<string,any>(f.inspection.currentMembers.map((row:any)=>[row.current.runId,row.current]))
    ...
    for(const existing of reconstructed.existing){const child=reconstructed.record.children.find((row:any)=>row.runId===existing.task.runId)!,task=currentByRun.get(existing.task.runId)!;git(f.repo,'worktree','add','-b',task.checkpoint!.branch,child.path,f.rootSha);const host=task.machineId===f.later.id?f.later.hostBindingDigest:f.receiver.hostBindingDigest;await runtime.createRun(runInput(task,await realpath(child.path),1,claimsByRun.get(task.runId)!,f.childArtifacts[task.issue-8]!,host,existing.authorityRequest,existing.checkpointRequest))}
    targetSpy=spyOn(f.dispatch,'verifiedSharedTarget').mockResolvedValue(f.target as any)
    checkpointSpy=spyOn(f.dispatch,'checkpointPolicyEnabled').mockResolvedValue(true)
    const checkpointOwner=await import('../src/checkpoints.ts')
    checkpointFlushSpy=spyOn(checkpointOwner,'flushRunCheckpoint').mockImplementation(async (run:any)=>{const expected=f.material.children.find((row:any)=>row.task.runId===run.runId)?.checkpointRequest??undefined;expect(run.checkpointIntent?.approvalRequest).toEqual(expected)})
    claimSpy=spyOn(f.dispatch,'sharedClaimForRun').mockImplementation(async (run:any)=>{const claim=claimsByRun.get(run.runId);if(!claim)throw Error('unexpected fixture run claim '+run.runId);return claim})
    ...
    const restoreTask=f.mutateCurrentTask(progressed.current.taskKey,(task:any)=>({...task,machineId:'altered-member'}));await expect(gateway.executeChildren({parent,groups,config:f.config,write:true},executionDeps)).rejects.toThrow(/task\/index mismatch|group succession/);restoreTask()
  ...
  }finally{process.env.PATH=previousPath;targetSpy?.mockRestore();claimSpy?.mockRestore();ghSpy?.mockRestore();checkpointSpy?.mockRestore();checkpointFlushSpy?.mockRestore();await f.cleanup()}
},30000)

test('status CLI keeps shared recovery read-only and never promotes a later claimed child',async()=>{
 const f:any=await realProgressedSuccessionFixture(),{runStatusCli}=await import('../src/status.ts'),configPath=join(f.home,'status-factory.json')
 const files=async(root:string):Promise<string[]>=>{const fs=await import('node:fs/promises'),rows:string[]=[];const walk=async(path:string,prefix='')=>{let entries:import('node:fs').Dirent[]=[];try{entries=await fs.readdir(path,{withFileTypes:true})}catch{return}for(const entry of entries){const relative=prefix+entry.name;if(entry.isDirectory())await walk(join(path,entry.name),relative+'/');else rows.push(relative)}};await walk(root);return rows.sort()}
 const output:string[]=[],log=spyOn(console,'log').mockImplementation(value=>{output.push(String(value))})
 try{
  await writeFile(configPath,JSON.stringify({executionMode:'shared',repos:[{repo:'acme/app',org:'acme',path:f.repo}]}))
  const beforeHead=f.stateHead,beforeMutations=f.mutations(),beforeFiles=await files(f.target.localRoot)
  const code=await runStatusCli(['--json','--config',configPath],f.home,{sharedTarget:async()=>f.target,gh:async args=>args.includes('search/issues')?JSON.stringify({items:[]}):f.gh(args),worktrees:async()=>[],logs:async()=>[],readLock:async()=>({held:false,pid:null})})
  expect(code).toBe(0);const report=JSON.parse(output.at(-1)!),rows=report.repos[0].recovery as Array<{taskKey:string;action:string;reason:string}>
  expect(rows.find(row=>row.taskKey===f.laterChild.taskKey)).toMatchObject({action:'refuse',reason:'current recovery role unavailable: complete coordination history unavailable'})
  expect(rows.find(row=>row.taskKey===f.unfinishedChild.taskKey)).toMatchObject({action:'refuse',reason:'current recovery role unavailable: complete coordination history unavailable'})
  expect(rows.some(row=>row.action==='recover')).toBe(false);expect(f.stateHead).toBe(beforeHead);expect(f.mutations()).toBe(beforeMutations);expect(await files(f.target.localRoot)).toEqual(beforeFiles)
 }finally{log.mockRestore();await f.cleanup()}
},30000)

test('status ignores ordinary claimed ownership instead of inventing a recovery refusal',async()=>{
  const {inspectSharedRecovery}=await import('../src/status.ts'),head='a'.repeat(40),shared={head,refusal:null,history:{coverage:'unavailable',archiveCoverage:'partial',sourceCommit:head},tasks:[{taskKey:'ordinary',repo:'acme/app',issue:7,state:'claimed',machineId:'box',generation:1,parentTaskKey:null,sourceCommit:head,originMachineId:'box',lastTransitionObservedAt:'2026-09-09T00:00:00Z',checkpoint:null,history:{coverage:'unavailable',events:[]}}]} as any
  let reads=0
  const rows=await inspectSharedRecovery(shared,{} as any,{task:async()=>{reads++;return{kind:'active',head,task:{schemaVersion:1,taskKey:'ordinary',repo:'acme/app',issue:7,state:'claimed',machineId:'box',generation:1}}},group:async()=>{throw Error('ordinary claim must not inspect succession')}} as any)
  expect(rows).toEqual([]);expect(reads).toBe(1)
})

test('provider-backed status recovers only the exact claimed parent and fails closed on receipt, endpoint or history drift',async()=>{
 const f:any=await realProgressedSuccessionFixture({statusClaimedAfterRecovery:true}),{runStatusCli}=await import('../src/status.ts'),configPath=join(f.home,'status-claimed.json'),output:string[]=[]
 const log=spyOn(console,'log').mockImplementation(value=>{output.push(String(value))})
 try{
  await writeFile(configPath,JSON.stringify({executionMode:'shared',repos:[{repo:'acme/app',org:'acme',path:f.repo}]}))
  const read=async()=>{output.length=0;const before=f.mutations(),code=await runStatusCli(['--json','--config',configPath],f.home,{sharedTarget:async()=>f.target,gh:async args=>args.includes('search/issues')?JSON.stringify({items:[]}):f.gh(args),worktrees:async()=>[],logs:async()=>[],readLock:async()=>({held:false,pid:null})});expect(code).toBe(0);expect(f.mutations()).toBe(before);return(JSON.parse(output.at(-1)!).repos[0].recovery??[]) as Array<{taskKey:string|null;action:string;reason:string}>}
  const exact=await read();expect(exact.find(row=>row.taskKey===f.parent.taskKey)).toMatchObject({action:'recover',reason:'exact current group parent verified; lifecycle checks required'});expect(exact.filter(row=>row.action==='recover')).toHaveLength(1);expect(exact.filter(row=>f.children.some((child:any)=>child.taskKey===row.taskKey)).every(row=>row.action==='wait')).toBe(true)
  const receipt=f.mutateGroupReceipt();const badReceipt=await read();expect(badReceipt.some(row=>row.action==='recover')).toBe(false);expect(badReceipt.some(row=>row.action==='refuse')).toBe(true);receipt()
  const parent=f.mutateCurrentTask(f.parent.taskKey,(task:any)=>({...task,ownerToken:randomUUID()}));const badParent=await read();expect(badParent.some(row=>row.action==='recover')).toBe(false);expect(badParent.some(row=>row.action==='refuse')).toBe(true);parent()
  const child=f.mutateCurrentTask(f.children[0].taskKey,(task:any)=>({...task,ownerToken:randomUUID()}));const badMember=await read();expect(badMember.some(row=>row.action==='recover')).toBe(false);expect(badMember.some(row=>row.action==='refuse')).toBe(true);child()
  for(const mode of ['missing-group','repeated-group'] as const){f.setHistoryMode(mode);const incomplete=await read();expect(incomplete.some(row=>row.action==='recover'),mode).toBe(false);expect(incomplete.some(row=>row.action==='refuse'),mode).toBe(true)}
  f.setHistoryMode('normal')
 }finally{log.mockRestore();await f.cleanup()}
},30000)

test('overlapping stopped-group controllers keep their fresh verifier contexts operation-scoped',async()=>{
 const f:any=await realProgressedSuccessionFixture({stoppedBeforeRecovery:true}),{processIdentity}=await import('../src/claims.ts'),{readBootIdentityDigest}=await import('../src/machine-identity.ts'),policyOwner=await import('../../../skills/dev/dev-setup/scripts/effective-policy.mjs'),{parseFactoryConfig}=await import('../src/config.ts'),room=join(f.home,'parallel-room'),settings=join(f.home,'parallel-factory.json')
 try{
  await mkdir(join(room,'groups','dev'),{recursive:true});const fleet={schemaVersion:1,coordination:f.receiver.coordination,defaults:{pollSeconds:120,maxRuns:2,childConcurrent:2,checkpoints:'task-branch',recovery:'verified-transfer'},groupDefaults:{},machines:{[f.receiver.id]:{installationId:f.receiver.installationId,hostBindingDigest:f.receiver.hostBindingDigest,executionLogin:'robot',group:'dev',repositories:['acme/app'],enabled:true,overrides:{}}}};await writeFile(join(room,'org.md'),'sync-max-age: 2h\npolicy-schema: 2\n```vsk-policy\n'+JSON.stringify({schemaVersion:2,fleet})+'\n```\n');await writeFile(join(room,'groups/dev/group.md'),'review: subagent\n');await writeFile(join(room,'people.csv'),'login,name,role,slack,timezone,groups\nrobot,Robot,lead,,UTC,dev\n');await writeFile(join(room,'repos.md'),'| repo | group | board | owner | repository-id |\n|---|---|---|---|---|\n| acme/app | dev | | robot | R_app |\n');git(room,'init','-b','main');git(room,'remote','add','origin','https://github.com/acme/control.git');git(room,'add','.');git(room,'commit','-m','policy');const devMd=await readFile(join(f.repo,'.vegastack/dev.md'),'utf8'),policySnapshot={schemaVersion:2,org:'acme',group:'dev',repository:'acme/control',origin:'https://github.com/acme/control.git',sourceCommit:git(room,'rev-parse','HEAD'),policyDigest:'0'.repeat(64),validatedAt:new Date().toISOString(),contentPath:room},loadedPolicy=policyOwner.loadSnapshotPolicy({snapshot:policySnapshot,repo:'acme/app',devMd,expectedOrigin:policySnapshot.origin,now:Date.now()});policySnapshot.policyDigest=loadedPolicy.policy.policyDigest;const raw={schemaVersion:2,revision:0,repos:[{repo:'acme/app',org:'acme',path:f.repo}],machine:{id:f.receiver.id,installationId:f.receiver.installationId,hostBindingDigest:f.receiver.hostBindingDigest,group:'dev',controlRoom:{repositoryId:'R_state',repo:'acme/control',remote:policySnapshot.origin,branch:'main'}},controlRooms:{acme:{repo:'acme/control',remote:policySnapshot.origin,path:room,branch:'main',sha:policySnapshot.sourceCommit,lastSyncedAt:policySnapshot.validatedAt,repositoryId:'R_state',snapshots:{'acme/app':policySnapshot}}}};await writeFile(settings,JSON.stringify(raw));const config=parseFactoryConfig(raw,f.home);config.settingsPath=settings
  const recoveryTarget={...await f.dispatch.verifiedSharedTarget('acme/app',config,undefined,f.gh),provider:f.target.provider},configured=(await import('../src/control-room.ts')).loadConfiguredPolicy({home:f.home,repo:'acme/app',devMd,settingsPath:settings}),resolved=policyOwner.resolveMachinePolicy({policy:configured.ok?configured.policy:null,machineId:f.receiver.id,installationId:f.receiver.installationId,hostBindingDigest:f.receiver.hostBindingDigest,executionLogin:'robot'}),receiver=resolved.machine;if(!resolved.ok||!receiver)throw Error('controlled receiver policy unavailable: '+JSON.stringify(resolved))
  let sourceRepositoryId='R_app',setupFingerprint='e'.repeat(64);const snapshot=await f.owner.readCoordination(f.target),tasks=Object.values(snapshot.tasks).filter((task:any)=>task.taskKey===f.parentTaskKey||task.parentTaskKey===f.parentTaskKey) as any[],source={repository:async()=>({node_id:sourceRepositoryId}),fetch:async()=>{}},materials=[] as any[]
  for(const task of tasks){const material=await f.dispatch.inspectRemoteRecovery({repo:'acme/app',taskKey:task.taskKey,config},{target:f.target,gh:f.gh,source});f.dispatch.assertRemoteRecoveryMaterial(material);materials.push(material)}
  const session={target:recoveryTarget,localRoot:recoveryTarget.localRoot,machineId:receiver.id,installationId:receiver.installationId,sessionId:randomUUID(),hostBindingDigest:receiver.hostBindingDigest,bootIdDigest:await readBootIdentityDigest(),identity:await processIdentity()},localClaim={path:'fixture',token:randomUUID(),identity:session.identity},checkouts=new Map(materials.map(row=>[row.task.taskKey,f.repo])),setup=async(_material:any,checkout:string)=>({receiver:{machine:{id:receiver.id,installationId:receiver.installationId,sessionId:session.sessionId,hostBindingDigest:receiver.hostBindingDigest},claimToken:localClaim.token,policyDigest:receiver.policyDigest,runtimeBinding:{schemaVersion:1,sourceSha:f.rootSha,treeSha:f.rootTree,packageName:'@vegastack/vegafactory',version:'1.0.0',tarballSha256:'6'.repeat(64),inventoryDigest:'7'.repeat(64)},configurationDigest:'d'.repeat(64),worktreeDigest:setupFingerprint},plan:{command:'node',args:[],env:{},cwd:checkout,prompt:''}}),setupDigests=new Map<string,string>()
  for(const material of materials)setupDigests.set(material.task.taskKey,createHash('sha256').update(f.owner.canonical(await setup(material,f.repo))).digest('hex'))
  let entered=0,releaseBoth!:()=>void,completeA!:()=>void;const both=new Promise<void>(resolve=>{releaseBoth=resolve}),aComplete=new Promise<void>(resolve=>{completeA=resolve}),operationA=randomUUID(),operationB=randomUUID()
  const run=(operationId:string,waitForA:boolean)=>{
   let current:any,inspection:any
   return f.dispatch.recoverStoppedGroupMaterials({operationId,parentTaskKey:f.parentTaskKey,materials,machine:receiver,session,runtime:{config,gh:f.gh,localClaim,checkouts,setupDigests,setup,source}},{
    recover:async({machine,session:scoped,request}:any)=>{if(++entered===2)releaseBoth();await both;if(waitForA)await aComplete;const receiver={machineId:machine.id,installationId:machine.installationId,sessionId:scoped.sessionId,hostBindingDigest:scoped.hostBindingDigest,bootIdDigest:scoped.bootIdDigest},rows=request.members.map((member:any,index:number)=>{const old=materials.find(row=>row.task.taskKey===member.expected.taskKey).task,next={...structuredClone(old),schemaVersion:2,parentBinding:old.parentBinding??null,successionOperationId:request.operationId,machineId:machine.id,installationId:machine.installationId,sessionId:scoped.sessionId,ownerToken:`${String(index+7).repeat(8)}-${String(index+7).repeat(4)}-4${String(index+7).repeat(3)}-8${String(index+7).repeat(3)}-${String(index+7).repeat(12)}`,generation:old.generation+1,state:old.taskKey===request.parentTaskKey?'claimed':'recovery-queued',recovery:{...structuredClone(old.recovery),generation:old.generation+1}};return{old,next}}),head=(operationId===operationA?'a':'b').repeat(40),reference={kind:'state-receipt',operationId,commitSha:head,blobSha256:'f'.repeat(64)},receipt={schemaVersion:2,type:'group-succession',operationId,parentTaskKey:request.parentTaskKey,previousHead:request.expectedHead,requestDigest:'c'.repeat(64),groupPlan:request.groupPlan,groupsDigest:request.groupsDigest,transferredAt:new Date().toISOString(),receiver,members:rows.map(({old,next}:any)=>({before:{taskKey:old.taskKey,runId:old.runId,generation:old.generation,ownerToken:old.ownerToken,machineId:old.machineId,installationId:old.installationId,sessionId:old.sessionId},after:{taskKey:next.taskKey,runId:next.runId,generation:next.generation,ownerToken:next.ownerToken,machineId:next.machineId,installationId:next.installationId,sessionId:next.sessionId},beforeTaskSha256:f.owner.sha256(f.owner.canonical(old)),afterTaskSha256:f.owner.sha256(f.owner.canonical(next)),previousSuccession:null}))};current={head,tasks:Object.fromEntries(rows.map((row:any)=>[row.next.taskKey,row.next]))};inspection={kind:'verified',reference,receipt,currentMembers:rows.map((row:any)=>({initial:row.next,current:row.next}))};await scoped.target.verifyGroupSuccession({parent:rows.find((row:any)=>row.old.taskKey===request.parentTaskKey).old,members:rows.map((row:any,index:number)=>({task:row.old,candidate:request.members[index].candidate})),groupPlan:request.groupPlan,groupsDigest:request.groupsDigest,machine,session:scoped});return{kind:'ambiguous',reason:'controlled overlapping readback'}},
    read:async()=>current,inspect:async()=>inspection,
   })
  }
  const first=run(operationA,false),second=run(operationB,true);void first.then(completeA,completeA);const [a,b]=await Promise.all([first,second]);expect(a).toMatchObject({kind:'owned',lostResponse:true});expect(b).toMatchObject({kind:'owned',lostResponse:true});expect(entered).toBe(2)
  const machineSettings=await readFile(settings,'utf8'),drifts:Array<[string,()=>()=>void]>=[
   ['machine-enrollment',()=>{const changed=JSON.parse(machineSettings);changed.machine.id='revoked';writeFileSync(settings,JSON.stringify(changed));return()=>writeFileSync(settings,machineSettings)}],
   ['approval',()=>f.mutateApproval()],
   ['dependency',()=>{f.setDependencies([{id:99,state:'open'}]);return()=>f.setDependencies([])}],
   ['source-checkpoint',()=>{sourceRepositoryId='R_changed';return()=>{sourceRepositoryId='R_app'}}],
   ['private-context',()=>{setupFingerprint='f'.repeat(64);return()=>{setupFingerprint='e'.repeat(64)}}],
   ['effect-evidence',()=>f.mutateEvidenceReceipt()],
  ]
  for(const [name,mutate] of drifts){const before=f.operations().length,operationId=randomUUID();await expect(f.dispatch.recoverStoppedGroupMaterials({operationId,parentTaskKey:f.parentTaskKey,materials,machine:receiver,session,runtime:{config,gh:f.gh,localClaim,checkouts,setupDigests,setup,source}},{recover:async({machine,session:scoped,request}:any)=>{const restore=mutate();try{await scoped.target.verifyGroupSuccession({parent:materials.find(row=>row.task.taskKey===request.parentTaskKey).task,members:request.members.map((member:any)=>({task:materials.find(row=>row.task.taskKey===member.expected.taskKey).task,candidate:member.candidate})),groupPlan:request.groupPlan,groupsDigest:request.groupsDigest,machine,session:scoped})}finally{restore()}return{kind:'refused',reason:'drift unexpectedly passed'}},read:async()=>{throw Error('drift reached readback')},inspect:async()=>{throw Error('drift reached inspection')}}),name).rejects.toThrow(/changed|blocked|differs|unavailable|refused/);expect(f.operations(),name).toHaveLength(before)}
 }finally{await f.cleanup()}
},60000)

test('real runTick resumes mixed receiving and same-home attempts after lost CAS readback, a partial start and a completed child phase',async()=>{
 const f:any=await realProgressedSuccessionFixture({stoppedBeforeRecovery:true,lostGroupReadback:true,sameHomeTask:'unfinished-child'}),runtime=await import('../src/runs.ts'),policyOwner=await import('../../../skills/dev/dev-setup/scripts/effective-policy.mjs'),{parseFactoryConfig}=await import('../src/config.ts'),room=join(f.home,'room'),settings=join(f.home,'factory.json'),previousPath=process.env.PATH??'',previousGh=process.env.VSK_GH;let ghServer:ReturnType<typeof Bun.serve>|null=null
 try{
  const actualGit=Bun.which('git')!,sourceRemote=join(f.home,'source.git'),bin=join(f.home,'git-bin'),merges=join(f.home,'merges');let command=spawnSync(actualGit,['init','--bare',sourceRemote],{encoding:'utf8'});if(command.status!==0)throw Error(command.stderr);command=spawnSync(actualGit,['push',sourceRemote,`${f.rootSha}:refs/heads/main`,`${f.rootSha}:refs/heads/task/1`,`${f.rootSha}:refs/heads/feat/9-child-9`],{cwd:f.repo,encoding:'utf8'});if(command.status!==0)throw Error(command.stderr);command=spawnSync(actualGit,['--git-dir',sourceRemote,'symbolic-ref','HEAD','refs/heads/main'],{encoding:'utf8'});if(command.status!==0)throw Error(command.stderr);await mkdir(bin);await writeFile(merges,'');await writeFile(join(bin,'git'),`#!${process.execPath}\nconst{appendFileSync}=require('node:fs'),{spawnSync}=require('node:child_process');const a=process.argv.slice(2).map(v=>v==='https://github.com/acme/app.git'?${JSON.stringify(sourceRemote)}:v);if(a[0]==='merge')appendFileSync(${JSON.stringify(merges)},'merge\\n');const r=spawnSync(${JSON.stringify(actualGit)},a,{stdio:'inherit',env:process.env});process.exit(r.status??1);\n`);await (await import('node:fs/promises')).chmod(join(bin,'git'),0o755);process.env.PATH=bin+':'+previousPath
  await mkdir(join(room,'groups','dev'),{recursive:true})
  const fleet={schemaVersion:1,coordination:f.receiver.coordination,defaults:{pollSeconds:120,maxRuns:1,childConcurrent:2,checkpoints:'task-branch',recovery:'verified-transfer'},groupDefaults:{},machines:{[f.receiver.id]:{installationId:f.receiver.installationId,hostBindingDigest:f.receiver.hostBindingDigest,executionLogin:'robot',group:'dev',repositories:['acme/app'],enabled:true,overrides:{}}}}
  await writeFile(join(room,'org.md'),'sync-max-age: 2h\npolicy-schema: 2\n```vsk-policy\n'+JSON.stringify({schemaVersion:2,fleet})+'\n```\n');await writeFile(join(room,'groups/dev/group.md'),'review: subagent\n');await writeFile(join(room,'people.csv'),'login,name,role,slack,timezone,groups\nrobot,Robot,lead,,UTC,dev\n');await writeFile(join(room,'repos.md'),'| repo | group | board | owner | repository-id |\n|---|---|---|---|---|\n| acme/app | dev | | robot | R_app |\n');git(room,'init','-b','main');git(room,'remote','add','origin','https://github.com/acme/control.git');git(room,'add','.');git(room,'commit','-m','policy')
  const devMd=await readFile(join(f.repo,'.vegastack/dev.md'),'utf8'),snapshot={schemaVersion:2,org:'acme',group:'dev',repository:'acme/control',origin:'https://github.com/acme/control.git',sourceCommit:git(room,'rev-parse','HEAD'),policyDigest:'0'.repeat(64),validatedAt:new Date().toISOString(),contentPath:room};snapshot.policyDigest=policyOwner.loadSnapshotPolicy({snapshot,repo:'acme/app',devMd,expectedOrigin:snapshot.origin,now:Date.now()}).policy.policyDigest
  const raw={schemaVersion:2,revision:0,repos:[{repo:'acme/app',org:'acme',path:f.repo}],machine:{id:f.receiver.id,installationId:f.receiver.installationId,hostBindingDigest:f.receiver.hostBindingDigest,group:'dev',controlRoom:{repositoryId:'R_state',repo:'acme/control',remote:snapshot.origin,branch:'main'}},controlRooms:{acme:{repo:'acme/control',remote:snapshot.origin,path:room,branch:'main',sha:snapshot.sourceCommit,lastSyncedAt:snapshot.validatedAt,repositoryId:'R_state',snapshots:{'acme/app':snapshot}}}}
  await writeFile(settings,JSON.stringify(raw));const config=parseFactoryConfig(raw,f.home);config.settingsPath=settings
  const gh=async(args:string[],options:any={})=>{
   const graphql=args.includes('graphql')&&args.includes('--input')
   if(graphql){const body=JSON.parse(options.input),query=body.query as string,variables=body.variables as any
    if(query.includes('createCommitOnBranch')){const additions=variables.input.fileChanges.additions as Array<{path:string;contents:string}>,result=await f.target.provider.commit(f.target,{branchId:variables.input.branch.id,expectedHeadOid:variables.input.expectedHeadOid,files:Object.fromEntries(additions.map(row=>[row.path,Buffer.from(row.contents,'base64').toString('utf8')])),operationId:variables.input.clientMutationId});if(result.kind==='committed')return JSON.stringify({data:{createCommitOnBranch:{commit:{oid:result.head}}}});if(result.kind==='conflict')return JSON.stringify({errors:[{type:'STALE_DATA'}]});throw Error('controlled conditional mutation response unavailable')}
    if(query.includes('object(expression:')){const split=variables.expression.indexOf(':'),at=variables.expression.slice(0,split),path=variables.expression.slice(split+1),text=await f.target.provider.read(f.target,at,path);return JSON.stringify({data:{repository:{object:text===null?null:{byteSize:Buffer.byteLength(text),isBinary:false,text}}}})}
    const branch=await f.target.provider.branch(f.target);return JSON.stringify({data:{repository:{id:branch.repositoryId,isPrivate:branch.private,defaultBranchRef:{name:branch.defaultBranch},ref:{id:branch.id,target:{oid:branch.head}}}}})
   }
   const compare=args.find(value=>value.startsWith('repos/acme/control/compare/'));if(compare){const [base,next]=compare.split('/compare/')[1]!.split('...');return JSON.stringify({status:await f.target.provider.compare(f.target,base!,next!)})}
   if(args.includes('user'))return JSON.stringify({login:'robot'})
   if(args.some(value=>value==='repos/acme/app/issues?state=open'))return 'HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n\r\n[]'
   return f.gh(args)
  }
  let setupCalls=0,injectPreCasDrift=true,restorePreCas:(()=>void)|null=null
  const target=await f.dispatch.verifiedSharedTarget('acme/app',config,undefined,gh),setup=async(material:any,checkout:string,_config:any,_target:any,localClaim:any,sessionId:string)=>{setupCalls++;if(injectPreCasDrift&&setupCalls===3)restorePreCas=f.mutateApproval();const prompt='controlled recovered group '+material.task.issue,parentCode=`require('node:fs').appendFileSync(${JSON.stringify(f.vendors)},'parent\\n')`;return{receiver:{machine:{id:f.receiver.id,installationId:f.receiver.installationId,sessionId,hostBindingDigest:f.receiver.hostBindingDigest},claimToken:localClaim.token,policyDigest:snapshot.policyDigest,runtimeBinding:{schemaVersion:1,sourceSha:f.rootSha,treeSha:f.rootTree,packageName:'@vegastack/vegafactory',version:'1.0.0',tarballSha256:'6'.repeat(64),inventoryDigest:'7'.repeat(64)},configurationDigest:'d'.repeat(64),worktreeDigest:await runtime.worktreeFingerprint(checkout)},plan:{command:Bun.which('node')!,args:['-e',parentCode,prompt],env:{},cwd:checkout,prompt}}}
  ;(target as any)[Symbol.for('vegafactory.test.stopped-group-receiving-setup')]=setup
  const tracker=new Map(),childCode=`require('node:fs').appendFileSync(${JSON.stringify(f.vendors)},'vendor\\n')`
  const deps={gh,tracker,shipGuard:async()=>({wired:true,detail:'controlled fixture'}),processDeps:{wrapperPath:join(source,'run-wrapper.ts')},recoveredChildPrepare:async(child:any,_record:any,_parent:any,_config:any)=>{if(child.runId!==f.sameHomeRunId)throw Error('accepted recovered child reached process preparation');const run=await runtime.readRun(runtime.runsRoot(f.home),child.runId);return{run,localClaim:null,plan:{command:Bun.which('node')!,args:['-e',childCode],env:{},cwd:run.checkout,prompt:''}}},recoveryTransport:async()=>({target,source:{repository:async()=>({node_id:'R_app'}),fetch:async()=>{}}})}
  const preCas=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker)
  expect(f.groupMutations,JSON.stringify(preCas)).toBe(0);expect(preCas.refusals.some((row:any)=>/source|authority|unverified|changed/.test(row.reason)),JSON.stringify(preCas.refusals)).toBe(true);if(restorePreCas)(restorePreCas as()=>void)();restorePreCas=null;injectPreCasDrift=false;setupCalls=0
  const lost=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker)
  expect(f.groupMutations,JSON.stringify(lost)).toBe(1);expect(setupCalls).toBeGreaterThan(3);expect((await runtime.readRuns(runtime.runsRoot(f.home))).filter((run:any)=>run.remoteRecovery?.kind==='receiving-group')).toEqual([]);expect(lost.refusals.some((row:any)=>row.reason.includes('readback'))).toBe(true)
  f.resumeReadback();const restorePostCas=f.mutateApproval(),postCas=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker)
  expect(f.groupMutations).toBe(1);expect((await runtime.readRuns(runtime.runsRoot(f.home))).filter((run:any)=>run.remoteRecovery?.kind==='receiving-group')).toEqual([]);expect(postCas.refusals.some((row:any)=>/source|authority|intent/.test(row.reason))).toBe(true);restorePostCas()
  ghServer=Bun.serve({port:0,fetch:async request=>{const body=await request.json() as {args:string[];input:string};return new Response(await gh(body.args,{input:body.input}))}});const ghBin=join(bin,'gh');await writeFile(ghBin,`#!${process.execPath}\nlet body='';process.stdin.setEncoding('utf8');process.stdin.on('data',x=>body+=x);process.stdin.on('end',async()=>{const r=await fetch('http://127.0.0.1:${ghServer.port}',{method:'POST',body:JSON.stringify({args:process.argv.slice(2),input:body})});process.stdout.write(await r.text())});\n`);await (await import('node:fs/promises')).chmod(ghBin,0o755);process.env.VSK_GH=ghBin
  const lockPath=f.dispatch.repoLockPath(config,'acme/app'),local=await f.dispatch.holdLock(lockPath,process.pid),beforeRecoveryOperations=f.operations().length,beforePartialStarts=f.operations().filter((row:any)=>row.type==='start').length
  const partial=await f.dispatch.prepareVerifiedStoppedGroup({repo:'acme/app',parentTaskKey:f.parentTaskKey,config,localClaim:local,target,gh,source:{repository:async()=>({node_id:'R_app'}),fetch:async()=>{}}})
  const partialRuns=(await runtime.readRuns(runtime.runsRoot(f.home))).filter((run:any)=>run.execution),partialIdentities=partialRuns.map((run:any)=>({runId:run.runId,attemptId:run.attemptId})).sort((a:any,b:any)=>a.runId.localeCompare(b.runId))
  expect(partial.started).toBe(true);expect(partialRuns).toHaveLength(3);expect(f.operations().filter((row:any)=>row.type==='start').slice(beforePartialStarts).map((row:any)=>row.taskKey)).toEqual([f.parentTaskKey])
  const priorStarts=f.operations().filter((row:any)=>row.type==='start').length,parentBeforeBarrier=await runtime.readRun(runtime.runsRoot(f.home),partial.run.runId),barrierPath=join(runtime.runsRoot(f.home),partial.run.runId,'controller-barrier.json'),barrier={schemaVersion:1,runId:partial.run.runId,attemptId:partial.run.attemptId??partial.run.runId,state:'active'};await runtime.atomicRunFile(barrierPath,barrier)
  const children=await import('../src/children.ts');await children.startRecoveredGroupMembers(partial.group.starts);const executed=await children.executeChildren({parent:parentBeforeBarrier,groups:partial.group,config,write:true},{processDeps:deps.processDeps,gh,prepare:deps.recoveredChildPrepare});expect(executed.blocked,JSON.stringify(executed)).toEqual([]);const joined=await children.joinChildren({parent:await runtime.readRun(runtime.runsRoot(f.home),partial.run.runId),groups:partial.group,config,write:true},{processDeps:deps.processDeps,gh});expect(joined.blocked,JSON.stringify(joined)).toEqual([]);expect(JSON.parse(await runtime.readPrivateRunFile(barrierPath))).toEqual(barrier);expect((await readFile(f.vendors,'utf8')).trim().split('\n')).toEqual(['vendor']);expect((await readFile(f.checks,'utf8')).trim().split('\n')).toEqual(['check','check']);expect(await readFile(merges,'utf8')).toBe('');await f.dispatch.releaseLock(lockPath,local)
  const heldLocks:string[]=[],walkLocks=async(path:string)=>{for(const row of await (await import('node:fs/promises')).readdir(path,{withFileTypes:true})){const child=join(path,row.name);if(row.isDirectory())await walkLocks(child);else if(row.name.endsWith('.lock')&&(await f.dispatch.readLock(child)).held)heldLocks.push(child.slice(f.home.length+1))}};await walkLocks(f.home);expect(heldLocks).toEqual([])
  const beforeReviewEdit={vendors:await readFile(f.vendors,'utf8'),checks:await readFile(f.checks,'utf8'),merges:await readFile(merges,'utf8'),starts:f.operations().filter((row:any)=>row.type==='start').length},restoreReview=f.mutateAcceptedReview(),editedReview=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker);expect(editedReview.refusals.some((row:any)=>row.issue===1),JSON.stringify(editedReview)).toBe(true);expect(editedReview.runs.every((row:any)=>!row.launched)).toBe(true);expect({vendors:await readFile(f.vendors,'utf8'),checks:await readFile(f.checks,'utf8'),merges:await readFile(merges,'utf8'),starts:f.operations().filter((row:any)=>row.type==='start').length}).toEqual(beforeReviewEdit);restoreReview()
  f.crashAfterNextChildComplete();const afterChildFinish=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker);expect(afterChildFinish.runs.some((row:any)=>row.launched),JSON.stringify(afterChildFinish)).toBe(true);expect((await runtime.readRun(runtime.runsRoot(f.home),partial.run.runId)).state).toBe('terminal');f.resumeReadback()
  const barrierBytes=await readFile(barrierPath,'utf8'),beforeMissingBarrier={vendors:await readFile(f.vendors,'utf8'),checks:await readFile(f.checks,'utf8'),merges:await readFile(merges,'utf8'),starts:f.operations().filter((row:any)=>row.type==='start').length};await rm(barrierPath);const missingBarrier=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker);expect(missingBarrier.refusals.some((row:any)=>row.issue===1&&row.reason.includes('barrier is missing')),JSON.stringify(missingBarrier)).toBe(true);expect({vendors:await readFile(f.vendors,'utf8'),checks:await readFile(f.checks,'utf8'),merges:await readFile(merges,'utf8'),starts:f.operations().filter((row:any)=>row.type==='start').length}).toEqual(beforeMissingBarrier);await writeFile(barrierPath,barrierBytes,{mode:0o600})
  const beforeFinishOnly={vendors:await readFile(f.vendors,'utf8'),checks:await readFile(f.checks,'utf8'),merges:await readFile(merges,'utf8'),starts:f.operations().filter((row:any)=>row.type==='start').length};f.blockAfterNextChildComplete();const blocked=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker);const blockedRun=await runtime.readRun(runtime.runsRoot(f.home),partial.run.runId),blockedParent=await f.owner.inspectCoordinationTask(target,f.parentTaskKey);expect(blockedRun.stopProof).not.toBeNull();expect(blockedParent.kind).toBe('active');if(blockedParent.kind!=='active')throw Error(blockedParent.reason);expect(blockedParent.task.state).toBe('blocked');expect(blocked.runs.every((row:any)=>!row.launched)).toBe(true);expect({vendors:await readFile(f.vendors,'utf8'),checks:await readFile(f.checks,'utf8'),merges:await readFile(merges,'utf8'),starts:f.operations().filter((row:any)=>row.type==='start').length}).toEqual(beforeFinishOnly);f.resumeReadback()
  f.crashBeforeNextParentStop();const afterProof=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker);const proofRun=await runtime.readRun(runtime.runsRoot(f.home),partial.run.runId),proofParent=await f.owner.inspectCoordinationTask(target,f.parentTaskKey);expect(proofRun.stopProof).not.toBeNull();expect(proofParent.kind).toBe('active');if(proofParent.kind!=='active')throw Error(proofParent.reason);expect(proofParent.task.state).toBe('blocked');expect(afterProof.runs.every((row:any)=>!row.launched)).toBe(true);expect({vendors:await readFile(f.vendors,'utf8'),checks:await readFile(f.checks,'utf8'),merges:await readFile(merges,'utf8'),starts:f.operations().filter((row:any)=>row.type==='start').length}).toEqual(beforeFinishOnly)
  const resumed=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker)
  const allRuns=await runtime.readRuns(runtime.runsRoot(f.home)),taskRuns=allRuns.filter((run:any)=>run.execution),remoteRuns=taskRuns.filter((run:any)=>run.remoteRecovery?.kind==='receiving-group'),sameHome=taskRuns.find((run:any)=>run.runId===f.sameHomeRunId)
  expect(f.groupMutations).toBe(1);expect(taskRuns,JSON.stringify({resumed,allRuns:allRuns.map((run:any)=>({runId:run.runId,issue:run.issue,state:run.state,headSha:run.headSha,checkpoint:run.checkpoint?.headSha,pending:run.pendingDelivery}))})).toHaveLength(3);expect(remoteRuns).toHaveLength(2);expect(new Set(taskRuns.map((run:any)=>run.runId)).size).toBe(3);expect(taskRuns.map((run:any)=>({runId:run.runId,attemptId:run.attemptId})).sort((a:any,b:any)=>a.runId.localeCompare(b.runId))).toEqual(partialIdentities);expect(remoteRuns.find((run:any)=>run.parent===null)?.state,JSON.stringify(resumed)).toBe('terminal');expect(sameHome?.remoteRecovery).toBeUndefined();expect(sameHome?.continuations).toHaveLength(1);expect(sameHome?.state,JSON.stringify(resumed.refusals)).toBe('terminal');expect(resumed.runs.every((row:any)=>!row.launched),JSON.stringify({resumed,taskRuns})).toBe(true);expect(resumed.refusals.some((row:any)=>row.issue===1),JSON.stringify(resumed.refusals)).toBe(false)
  const starts=f.operations().filter((row:any)=>row.type==='start').slice(priorStarts),acceptedKey=f.parentRecovery.children[0].childTaskKey
  expect(starts.some((row:any)=>row.taskKey===acceptedKey)).toBe(false);expect(starts).toHaveLength(1);expect(f.operations().filter((row:any)=>row.type==='start').slice(beforePartialStarts).map((row:any)=>row.taskKey)).toEqual([f.parentTaskKey,starts[0]!.taskKey]);for(const old of f.oldClaims){const before=f.operations().length,result=await f.owner.transitionSharedTask({claim:old,operationId:randomUUID(),transition:{kind:'start'}});expect(result.kind).not.toBe('owned');expect(f.operations()).toHaveLength(before)}
  const parentFinishes=f.operations().slice(beforeRecoveryOperations).filter((row:any)=>row.taskKey===f.parentTaskKey&&['block','stop'].includes(row.type));expect(parentFinishes.map((row:any)=>row.type)).toEqual(['block','stop']);expect(new Set(parentFinishes.map((row:any)=>row.operationId)).size).toBe(2)
  const accepted=await f.owner.inspectCoordinationTask(target,acceptedKey);expect(accepted.kind).toBe('completed');if(accepted.kind!=='completed')throw Error(accepted.reason);expect(accepted.task.state).toBe('completed');expect(accepted.task.stopProof?.generation).toBe(accepted.task.generation);expect(accepted.task.acceptedScopes.some((row:any)=>row.scopeDigest===accepted.task.scopeDigest)).toBe(true)
  expect(JSON.parse(await runtime.readPrivateRunFile(barrierPath)).state).toBe('complete');expect((await readFile(f.vendors,'utf8')).trim().split('\n')).toEqual(['vendor','parent']);expect((await readFile(f.checks,'utf8')).trim().split('\n')).toEqual(['check','check']);expect(await readFile(merges,'utf8')).toBe('')
  const identities=taskRuns.map((run:any)=>({runId:run.runId,attemptId:run.attemptId})).sort((a:any,b:any)=>a.runId.localeCompare(b.runId)),third=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker)
  const replay=(await runtime.readRuns(runtime.runsRoot(f.home))).filter((run:any)=>run.execution).map((run:any)=>({runId:run.runId,attemptId:run.attemptId})).sort((a:any,b:any)=>a.runId.localeCompare(b.runId))
  expect(f.groupMutations).toBe(1);expect(f.operations().filter((row:any)=>row.type==='start')).toHaveLength(priorStarts+1);expect(replay).toEqual(identities);expect(third.runs.every((row:any)=>!row.launched)).toBe(true);expect((await readFile(f.vendors,'utf8')).trim().split('\n')).toEqual(['vendor','parent']);expect((await readFile(f.checks,'utf8')).trim().split('\n')).toEqual(['check','check']);expect(await readFile(merges,'utf8')).toBe('')
  const settingsBytes=await readFile(settings,'utf8'),orgPath=join(room,'org.md'),orgBytes=await readFile(orgPath,'utf8'),rotatedInstallation=randomUUID(),rotatedSettings=JSON.parse(settingsBytes),beforeRevokedOperations=f.operations().length;await writeFile(orgPath,orgBytes.replace(f.receiver.installationId,rotatedInstallation));git(room,'add','org.md');git(room,'commit','-m','rotate fixture installation');const rotatedSnapshot={...snapshot,sourceCommit:git(room,'rev-parse','HEAD'),validatedAt:new Date().toISOString(),policyDigest:'0'.repeat(64)};rotatedSnapshot.policyDigest=policyOwner.loadSnapshotPolicy({snapshot:rotatedSnapshot,repo:'acme/app',devMd,expectedOrigin:rotatedSnapshot.origin,now:Date.now()}).policy.policyDigest;rotatedSettings.machine.installationId=rotatedInstallation;rotatedSettings.controlRooms.acme.sha=rotatedSnapshot.sourceCommit;rotatedSettings.controlRooms.acme.snapshots['acme/app']=rotatedSnapshot;await writeFile(settings,JSON.stringify(rotatedSettings));await f.dispatch.verifiedSharedTarget('acme/app',config,sameHome!.runId,gh);await expect(f.dispatch.sharedClaimForRun(sameHome!,config,gh)).rejects.toThrow('current shared run machine authorization differs');expect(f.operations()).toHaveLength(beforeRevokedOperations);expect((await readFile(f.vendors,'utf8')).trim().split('\n')).toEqual(['vendor','parent']);expect((await readFile(f.checks,'utf8')).trim().split('\n')).toEqual(['check','check']);expect(await readFile(merges,'utf8')).toBe('')
 }finally{ghServer?.stop(true);process.env.PATH=previousPath;if(previousGh===undefined)delete process.env.VSK_GH;else process.env.VSK_GH=previousGh;await f.cleanup()}
},240000)
...
test('stopped-group restart starts the parent before only unfinished recovery-queued children',async()=>{
 const {startRecoveredGroupMembers}=await import('../src/children.ts'),operationId=randomUUID(),parentKey='a'.repeat(64)
 const claim=(taskKey:string,runId:string)=>({taskKey,runId,generation:2,ownerToken:randomUUID(),machineId:'receiver',installationId:'11111111-1111-4111-8111-111111111111',sessionId:'22222222-2222-4222-8222-222222222222',stateCommit:'f'.repeat(40),target:{}} as any)
 const parentClaim=claim(parentKey,'33333333-3333-4333-8333-333333333333'),acceptedClaim=claim('b'.repeat(64),'44444444-4444-4444-8444-444444444444'),unfinishedClaim=claim('c'.repeat(64),'55555555-5555-4555-8555-555555555555')
 const acceptedJoin={state:'accepted',childRunId:acceptedClaim.runId,parentAfter:'1'.repeat(40),acceptance:{sourceSha:'1'.repeat(40)}}
 const parentTask={schemaVersion:2,state:'claimed',...parentClaim,parentTaskKey:null,recovery:{joins:[acceptedJoin]},successionOperationId:operationId}
 const child=(value:any)=>({schemaVersion:2,state:'recovery-queued',...value,parentTaskKey:parentKey,parentBinding:{taskKey:parentKey,runId:parentClaim.runId,generation:1,ownerToken:'66666666-6666-4666-8666-666666666666',machineId:'old',installationId:'77777777-7777-4777-8777-777777777777',sessionId:'88888888-8888-4888-8888-888888888888'},successionOperationId:operationId})
 const calls:string[]=[]
 const result=await startRecoveredGroupMembers({parent:{task:parentTask as any,claim:parentClaim,startOperationId:'99999999-9999-4999-8999-999999999999'},children:[{task:child(acceptedClaim) as any,claim:acceptedClaim,startOperationId:randomUUID()},{task:child(unfinishedClaim) as any,claim:unfinishedClaim,startOperationId:'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'}]},async({claim,transition})=>{if(transition.kind==='start')calls.push(claim.runId);return{kind:'owned',claim} as any})
 expect(calls).toEqual([parentClaim.runId,unfinishedClaim.runId])
 expect(result.parent.runId).toBe(parentClaim.runId);expect(result.children.map(row=>row.runId)).toEqual([unfinishedClaim.runId]);expect(result.acceptedRunIds).toEqual([acceptedClaim.runId])
 const bad=child(unfinishedClaim) as any;bad.state='blocked';calls.length=0
 await expect(startRecoveredGroupMembers({parent:{task:parentTask as any,claim:parentClaim,startOperationId:randomUUID()},children:[{task:child(acceptedClaim) as any,claim:acceptedClaim,startOperationId:randomUUID()},{task:bad,claim:unfinishedClaim,startOperationId:randomUUID()}]},async({claim})=>{calls.push(claim.runId);return{kind:'owned',claim} as any})).rejects.toThrow('recovery-queued')
