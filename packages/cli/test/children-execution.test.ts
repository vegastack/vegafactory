...
test('real runTick resumes mixed receiving and same-home attempts after lost CAS readback and a partial start',async()=>{
 ...
 try{
  ...
  const settingsBytes=await readFile(settings,'utf8'),revokedSettings=JSON.parse(settingsBytes),beforeRevokedOperations=f.operations().length,beforeRevokedStarts=f.operations().filter((row:any)=>row.type==='start').length;revokedSettings.machine.id='revoked';await writeFile(settings,JSON.stringify(revokedSettings));await expect(f.dispatch.sharedClaimForRun(partial.run,config,gh)).rejects.toThrow(/machine|scope/);expect(f.operations()).toHaveLength(beforeRevokedOperations);expect(f.operations().filter((row:any)=>row.type==='start')).toHaveLength(beforeRevokedStarts);expect((await readFile(f.vendors,'utf8').catch(()=>''))).toBe('');expect((await readFile(f.checks,'utf8').catch(()=>''))).toBe('');await writeFile(settings,settingsBytes);expect(JSON.parse(await readFile(settings,'utf8')).machine.id).toBe(f.receiver.id)
