...
test('real runTick resumes mixed receiving and same-home attempts after lost CAS readback and a partial start',async()=>{
 ...
 try{
  ...
  expect(partial.started).toBe(true);expect(partialRuns).toHaveLength(3);expect(f.operations().filter((row:any)=>row.type==='start').slice(beforePartialStarts).map((row:any)=>row.taskKey)).toEqual([f.parentTaskKey]);await f.dispatch.releaseLock(lockPath,local)
  ...
  const priorStarts=f.operations().filter((row:any)=>row.type==='start').length,resumed=await f.dispatch.runTick(config,{dryRun:false},deps as any);await f.dispatch.settleRuns(tracker)
