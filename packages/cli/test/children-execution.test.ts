...
async function realProgressedSuccessionFixture(){
  const owner=await import('../src/shared-claims.ts'),{processIdentity}=await import('../src/claims.ts'),home=await realpath(await mkdtemp(join(tmpdir(),'child-succession-real-'))),rootSha='1'.repeat(40),digest='d'.repeat(64),installation=randomUUID()
  ...
  const authority={approvalId:'scope',source:{kind:'github-comment' as const,repositoryId:'R_app',issueNodeId:'I_parent',commentId:'12',bodySha256:digest}},parentCandidate={host:'github.com',repo:'acme/app',issue:1,repositoryNodeId:'R_app',issueNodeId:'I_1',scopeDigest:digest,approvalDigest:digest,approvalBindings:[authority],runId:randomUUID(),stage:'implement',paths:[],resources:[],independent:false,parentTaskKey:null,approvedTaskIds:['1-T1']}
  ...
  const childCandidates=[8,9].map((issue,index)=>({...parentCandidate,issue,issueNodeId:'I_'+issue,runId:randomUUID(),paths:['src/child-'+index],independent:true,parentTaskKey:originalParent.taskKey,parentBinding:originalParent,approvedTaskIds:[issue+'-T1']})),childClaims=[] as import('../src/shared-claims.ts').SharedClaim[]
  ...
  return{home,owner,target,inspection,parentRecovery,acceptedRunId:childA.claim.runId,unfinishedRunId:childClaims[1]!.runId,cleanup:()=>rm(home,{recursive:true,force:true})}
}
...
test('real later child handoff preserves accepted join history and schedules only unfinished work',async()=>{
  const f=await realProgressedSuccessionFixture(),gateway=await import('../src/children.ts') as typeof import('../src/children.ts')&{reconcileProgressedChildren?:(inspection:any,recovery:any)=>any}
  try{
    ...
    expect(typeof gateway.reconcileProgressedChildren).toBe('function')
