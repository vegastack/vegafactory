...
test('overlapping stopped-group controllers keep their fresh verifier contexts operation-scoped',async()=>{
 ...
 try{
  ...
  const session={target:f.target,localRoot:f.target.localRoot,machineId:f.receiver.id,installationId:f.receiver.installationId,sessionId:randomUUID(),hostBindingDigest:f.receiver.hostBindingDigest,bootIdDigest:'d'.repeat(64),identity:await processIdentity()},localClaim={path:'fixture',token:randomUUID(),identity:session.identity},checkouts=new Map(materials.map(row=>[row.task.taskKey,f.repo])),setup=async(_material:any,checkout:string)=>({receiver:{machine:{id:f.receiver.id,installationId:f.receiver.installationId,sessionId:session.sessionId,hostBindingDigest:f.receiver.hostBindingDigest},claimToken:localClaim.token,policyDigest:f.receiver.policyDigest,runtimeBinding:{schemaVersion:1,sourceSha:f.rootSha,treeSha:f.rootTree,packageName:'@vegastack/vegafactory',version:'1.0.0',tarballSha256:'6'.repeat(64),inventoryDigest:'7'.repeat(64)},configurationDigest:'d'.repeat(64),worktreeDigest:setupFingerprint},plan:{command:'node',args:[],env:{},cwd:checkout,prompt:''}}),setupDigests=new Map<string,string>()
  ...
  const run=(operationId:string,waitForA:boolean)=>{
   ...
   return f.dispatch.recoverStoppedGroupMaterials({operationId,parentTaskKey:f.parentTaskKey,materials,machine:f.receiver,session,runtime:{config,gh:f.gh,localClaim,checkouts,setupDigests,setup,source}},{
  ...
  const drifts:Array<[string,()=>()=>void]>=[
...
test('real runTick resumes mixed receiving and same-home attempts after lost CAS readback and a partial start',async()=>{
 ...
 try{
  ...
  const target=await f.dispatch.verifiedSharedTarget('acme/app',config,undefined,gh),setup=async(material:any,checkout:string,_config:any,_target:any,localClaim:any,sessionId:string)=>{setupCalls++;if(injectPreCasDrift&&setupCalls===3)restorePreCas=f.mutateApproval();const prompt='controlled recovered group '+material.task.issue,parentWait=`const fs=require('node:fs'),started=Date.now(),file=${JSON.stringify(f.checks)};const timer=setInterval(()=>{let count=0;try{count=fs.readFileSync(file,'utf8').trim().split('\\n').filter(Boolean).length}catch{}if(count>=2){clearInterval(timer);setTimeout(()=>process.exit(0),5000)}else if(Date.now()-started>60000){clearInterval(timer);process.exit(9)}},50)`;return{receiver:{machine:{id:f.receiver.id,installationId:f.receiver.installationId,sessionId,hostBindingDigest:f.receiver.hostBindingDigest},claimToken:localClaim.token,policyDigest:snapshot.policyDigest,runtimeBinding:{schemaVersion:1,sourceSha:f.rootSha,treeSha:f.rootTree,packageName:'@vegastack/vegafactory',version:'1.0.0',tarballSha256:'6'.repeat(64),inventoryDigest:'7'.repeat(64)},configurationDigest:'d'.repeat(64),worktreeDigest:await runtime.worktreeFingerprint(checkout)},plan:{command:Bun.which('node')!,args:['-e',parentWait,prompt],env:{},cwd:checkout,prompt}}}
