...
type Command = 'add' | 'update' | 'verify' | 'doctor' | 'remove' | 'list' | 'version' | 'help' | 'worktree' | 'sync' | 'hook' | 'ship' | 'issue' | 'init'
...
function parse(argv: string[]): Options {
  ...
  if (argv[0] && !argv[0].startsWith('-')) {
    ...
    else if (head === 'worktree' || head === 'hook' || head === 'ship' || head === 'issue') return { command: head, all: false, dryRun: false, force: false, nonInteractive: false, json: false, rest: argv.splice(0) }
