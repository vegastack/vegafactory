...
| `ship release <n>` | Tag the merged release on issue n's recorded "ship it": it checks the word, the version and its changelog entry, then creates and pushes `v<version>`. It never publishes — the tag-triggered workflow does |
