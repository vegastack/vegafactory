...
**Where the logs are.** Both platforms write `worker.log` and `worker.err.log` into the repository's own `.vegastack/.tmp/worker/` — beside the checkout the worker polls, not in the home directory. On Linux those two files are the whole of the worker's output: the unit redirects its streams, so `journalctl --user -u vegafactory-worker.service` shows systemd's own messages about the service starting and stopping, and nothing the worker itself printed.
...
The wait is read from the box rather than guessed. logind keeps a user's manager alive for a
grace period after the last session ends, so a worker with no linger at all still answers `active`
inside that window — the mechanism, the default and where it is documented are in the
vegafactory-setup skill's `references/worker-host.md`. The
...
documented default on a box that had been given a longer one is how a worker with no linger passes
