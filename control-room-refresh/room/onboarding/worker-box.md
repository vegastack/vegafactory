...
**Where the logs are.** Both platforms write `worker.log` and `worker.err.log` under `~/.vegafactory/worker/`. Linux keeps everything in the journal as well — `journalctl --user -u vegafactory-worker.service` — and the journal is the one that survives the files being cleaned up.

Then the reboot drill: `sudo reboot`, wait for the box, and run every check above again **without logging anything in by hand**. On Linux, log out and back in once as well, and check the worker is still running: the reboot proves it starts, and only the logout proves linger. A box that needs a human at the keyboard after a power cut is not always-on.
