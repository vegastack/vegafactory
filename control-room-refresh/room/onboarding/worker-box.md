...
3. A listed repository is cloned on first use into `~/.vegafactory/worker/repos/<owner>__<repo>/repo`. The clone is accepted atomically only after its origin, Git shape, standard Claude and Codex hooks and machine-login push path pass; an existing ambiguous path is reported and left untouched.
...
- **A worker run pushes with this box's own login, never with its board's App token** — the App's Contents is read-only, and the gh credential helper would hand a run the App's token instead of the login this box already has. A run's git asks with that token scrubbed out, so `gh auth login` on this machine is all an https remote needs, and an SSH remote never asked a helper anyway. Every board's readiness check asks from its own worker-owned clone and skips only that board when its push path cannot be proved.
