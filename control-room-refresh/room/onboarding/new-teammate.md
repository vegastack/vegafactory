...
5. Run `vegafactory init`, then follow `onboarding/new-node.md`: send its proposed `worker: no` row to an operator for a control-room PR. Init never writes or pushes the room.
6. Subscribe to the org's Slack channel through the official GitHub Slack app; notifications are GitHub assignment plus Slack, nothing else.
7. Add the login to the group's `operators:` line if they will own issues in that group.
