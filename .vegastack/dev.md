...
- CI (pull requests and the merge queue) runs on GitHub-hosted `ubuntu-latest`: a public repo's pull request can change any workflow, so PR code never runs on the self-hosted Mac mini. Fork workflow runs need a maintainer's approval. The Mac mini org runners (`vsk-runners-mac-mini`, macOS user `vegastack-runners`) run only trusted jobs such as the board mirror and, until #214 lands, release preparation
