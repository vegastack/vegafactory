...
- guard: release preparation uses `node scripts/release-artifacts.mjs prepare work/release v<version>` from clean reviewed source; build dashboard/CLI once, scan before packing, pack dashboard then generate the CLI descriptor then pack CLI; retain pair manifest, exact tarballs, toolchains, actual platform smokes and build/runtime SBOMs. Publication never rebuilds.
- guard: `node scripts/release-publish.mjs work/release/release-manifest.json --publish --promote` belongs only to the authorized release; stage dashboard then CLI under a candidate tag, reconcile registry integrity and downloaded bytes, smoke first use before promoting both. An outage is never absence; preserve partial versions and resume only matching bytes. Provenance/authentication remain the later #156 release decision, and current runners stay.
