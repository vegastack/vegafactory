...
- guard: a Release workflow run has exactly one executable attempt because GitHub removes prior-attempt artifacts when a run is rerun. Any preparation, publication, smoke, or promotion failure is diagnosed from retained evidence and rolled forward through a new patch PR and tag; rerunning the same run fails before preparation, never rebuilds or republishes, and never treats an outage as absence. Latest never moves backward, and local live publication is refused.
