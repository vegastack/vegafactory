...
- **What:** A dispatcher's caps — how many runs at once, how long one may take, how often the board is read, how long a failure waits, how many failures park an issue — now come from a `caps` cell on its roster row instead of being fixed in the code. `runs 10 · step 72h · poll 1m` on this machine's row, read live, no restart.
- **Why:** The first live drill found every one of them a module constant. Letting a real feature take longer than twenty minutes, or pushing a box harder, meant editing source and cutting a release. The caps belong to the machine — its processor, its subscription — so they belong beside the row that authorises it, not in any project's dev.md.
- **How it went:** The caps landed easily; the multi-repository half that shipped with them did not. Three review rounds found twenty-one defects in it — the checkout, the token and the hand-back all still belonging to the repository the dispatcher started in, then a board removed from the row leaving its runs going, then a registered path that is a worktree rather than a checkout, and `Org/Repo` and `org/repo` being two boards here and one to GitHub. Each round reached a layer the last had not. That is the shape of a change that wants its own plan, so it went back to intake and the caps landed alone.
- **Changed:** `Caps` and `parseCaps` on the roster row, with per-field units, a refusal for a cap nobody can read and a refusal for a delay no timer can hold · the caps reaching the scheduler, the retry backoff, the park count, the poll interval and the step watchdog · re-read from the refreshed roster every pass · a warning when the step outlives an installation token · the roster template, the control-room reference and the dispatcher-box checklist. Two fixes found alongside: a failed run goes back to the state it came from instead of staying `in-progress`, and the template's header row is no longer read as a machine called `dispatcher`.
