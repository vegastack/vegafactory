...
- **What:** The worker names the worktrees that could be reclaimed, every pass; `vegafactory worktree prune --write` reclaims them. Dependencies go first, on a shorter window, and the checkout says so until somebody reinstalls them.
...
- **How it went:** The review found five ways it did not work, and the last changed the answer's shape: removing on its own needs to know whether a person is sitting in a checkout, and nothing tells you. The run map is blind to attended sessions, an mtime does not move for someone reading, and git's worktree lock is not reference-counted — two sessions in one checkout have the first to end release the other's hold. That was built and reverted, being worse than nothing. So the pass reports and a person reclaims. Two halves went to their own issues: the threads work to [#274](https://github.com/vegastack/vegafactory/issues/274), because a `--session-id` that starts a session is not a resume, and putting dependencies back to [#275](https://github.com/vegastack/vegafactory/issues/275) — twelve review rounds found something real in that path every time and nothing in this one, because reinstalling is not a cleanup question but a scheduling one, inside the run loop where budgets and stops apply.
- **Changed:** a `worktree-deps-retention` knob · dependencies dropped on `prune --write`, with the checkout saying what to run · the worker reporting what could go and what it would refuse.
