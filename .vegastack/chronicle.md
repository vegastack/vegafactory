...
- **What:** `vegafactory worktree prune --write` reclaims what idle worktrees are holding. Dependencies go first, on a shorter window, and the checkout says which `setup` command puts them back.
...
- **How it went:** Five review cycles, and the shape of the answer changed twice. First: having the worker *remove* on its own needs to know whether a person is sitting in a checkout, and nothing tells you — not the run map, not an mtime, not git's worktree lock, which is not reference-counted, so two sessions in one checkout have the first to end release the other's hold. That was built and reverted, being worse than nothing. Then the same seam again, one level up: having the worker *notice* on its own is a scheduling problem in the poll loop, not a cleanup problem, and every finding after cycle 4 was about that integration rather than about reclaiming. Three halves went to their own issues — agent threads to [#274](https://github.com/vegastack/vegafactory/issues/274), putting dependencies back to [#275](https://github.com/vegastack/vegafactory/issues/275), the worker's own pass to [#276](https://github.com/vegastack/vegafactory/issues/276) — and what is left is a person's command that converged.
- **Changed:** a `worktree-deps-retention` knob · dependencies dropped on `prune --write`, with the checkout naming what to run · closed and merged worktrees finished without waiting out a window · a prune that no longer recreates a branch a squash merge deleted · the worker's records owner-only, directory and files.
- **Decisions:** one proposed and not yet in the register — reclaiming a worktree is a person's command, never a background pass — because the register takes the operator's own word and it has not been given.
...
- **How it went:** The identity became one fail-closed pair, and the worker's child keeps those two non-secret values while still losing the private-key path. The shipped guide and template-output test were narrowed to examples and shape rather than VegaStack-only numbers.
...
- **Why:** P12 is the phase that turns "passes tests and review" into "works", and the dispatcher was the last thing in 0.20 proven only by tests. Running it for the first time, on the box the rebuild was built on, broke it in the first ten minutes.
- **How it went:** The Codex probe passed `-a never`, a flag `codex exec` does not have, so that check had never once succeeded anywhere. The push fix took two attempts: the first killed git's credential helper outright and invented an SSH requirement, when the real problem was the App's token shadowing a login the machine already had. Asking for a credential with that token scrubbed gets the machine's own back, and readiness now asks the same way instead of insisting on a transport.
- **Changed:** the Codex readiness probe and stdin handling on all probes · a run's git credential path and the `push` readiness check · the App and dispatcher-box docs that described the SSH requirement.
- **Decisions:** none — two corrections, no new direction.
...
- **Changed:** Paired `0.19.0` metadata · exact major-transition authority guard · required CI and release-preparation enforcement · generated release-entry recognition in the ship gate · no product behavior rollback.
- **Decisions:** none.
