...
- **Why:** Its repeated gates and adjudication loops cost more time and context than the operator wanted, while deterministic tests and independent security review remain.
- **How it went:** The removal was treated as a source-tree contract, not a disabled switch: authored code, hidden configuration, generated-package wiring, current tracked history, templates, fixtures, and parser-specific workarounds all joined the same zero-reference audit.
- **Changed:** one published skill removed · two profile keys removed · merge-queue setup and artifact steps removed · direct code review remains.
- **Decisions:** none.

— approved by (kmanojkumar) · built by codex · branch chore/286-remove-external-skill-inspection
