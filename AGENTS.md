...
- **Verification runs in three layers, each once, behind the merge queue.** The commit-msg hook runs `bun run check:fast` (skipped for `wip:` checkpoints); while building run `bun run test:affected`; the queue runs the full `bun run check` on main plus the PR, with the pack smoke test and the skill scan. The SkillSpector scan runs only there: CI installs one hash-locked commit and runs it with `--no-provision`, while a local run self-installs and upgrades through this machine's own channel under dev.md's `skillspector-update: auto`. Either way it needs Python 3.12 and reads the **built** bundle, because the authored tree's test fixtures are adversarial on purpose. Run it locally only to investigate a finding — invocation in dev.md's `## Verify`.
