...
This repo *is* the factory it ships, and it runs on it: `.vegastack/` is a live instance of the workflow described above, so the process docs here are also a worked example of the product. The `vegafactory` on your PATH is the released one — build before you trust a change to it.

- **The CLI is the way in and out of an issue.** Read from the cache under `.vegastack/.tmp/issues/` and write with `vegafactory issue`; claim before building and let the hook keep the heartbeat; review with `vegafactory review <n>`; gate the merge with `ship check <n>` and the tag with `ship release <n>`. `vegafactory hook <event> --harness claude|codex` is the one hook command, and it carries the ship guard — the always-ask list plus dev.md's `ask:` lines, read from the default branch, so editing dev.md in a branch loosens nothing.
...
- **Authoring and standards:** new or changed skills go through `skillify`, which scaffolds the tree and performs the repo wiring itself; repo and release standards live in `skill-maintainer`. Both are listed in `packages/cli/repo-only.json`, so `add --all` skips them.
- **Verification runs in three layers, each once, behind the merge queue.** The commit-msg hook runs `bun run check:fast` (skipped for `wip:` checkpoints); while building run `bun run test:affected`; the queue runs the full `bun run check` on main plus the PR, with the pack smoke test. Nothing re-runs after merge.
