...
Use `policy-schema: 2` and one `vsk-policy` fenced JSON object in `org.md`. Its `schemaVersion` is2, with `locked` and `delegations` as shown in the org template. Lockable keys are `stats`, `stats-export`, `tests`, `review`, `provider-mode`, `learning`, `learning-adoption`. A delegation names exact groups, canonical repositories and allowed values; wildcards and delegation chains refuse. Attributed reporting requires explicit org authorization. Unknown extension fields remain inert; duplicate keys, bad known values or an unknown schema refuse rather than overwrite.
