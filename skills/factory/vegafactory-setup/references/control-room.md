...
- `groups/<g>/group.md` carries one default for every knob a group can decide, in the same line shape a dev.md uses — `harness-policy:` is one line, never six `harness:` lines. Per-repo facts (`repo:`, `board:`, `control-room:`, detected types and fields) have no group default.
...
`stats/` is the one tree automation writes, and its shape is the reason it can. One file per **owner**, per **node**, per day means two OS users on one host never touch the same file even when both use the same GitHub login, so a concurrent push is a non-fast-forward — solved by a rebase and a retry — and never a content conflict needing a human. The owner is who did the work, read from that machine's own `gh` login; the node is `<os-user>@<hostname>`. The `@` is written as `-` in the filename and the record keeps the full form, so the readable part of the name is a convenience — the twelve-character digest of the exact owner and node is what keeps two identities off one file, since rewriting and truncation alone would let `a-b@c` and `a@b-c` meet. Nothing here is summarised in the repository: a summary that accumulated would drift the first time a record arrived late from a machine that was offline, so totals are computed when they are read.
