...
- `dispatchers.md` is the roster of machines allowed to work a board with nobody at the keyboard: one row per machine, `| machine | group | repos | owner | caps | notes |`, the machine name being its lowercase hostname without the `.local` suffix. The `caps` cell sets what it may do — `runs 10 · step 72h · poll 1m · retry 15m · park 3`, every field optional, found by what it says rather than which column it sits in — and is re-read every pass, so a change lands on the next poll. A machine that is not listed, or whose row is edited only in a local copy, refuses every dispatcher verb. Its run caps, its retry deadlines and the seconds between reserving an issue and the run's own claim are per machine today — two listed machines on one repository can each run their full number of steps at once and can each start work the other is already holding in that window (tracked as fleet-wide leases).
