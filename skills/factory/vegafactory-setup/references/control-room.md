...
- `dispatchers.md` is the roster of machines allowed to work the board with nobody at the keyboard: one row `| machine | operator | repos |`, where the machine name is its hostname in lowercase without `.local`, and `*` in the repos cell means every repository of the org. `vegafactory dispatch` refuses every verb but `disable` on a machine this file does not name, so adding a row is the enrolment and deleting one is the stand-down — both operator PRs, never an agent's edit. Nothing about a machine's credentials belongs here; the App key lives on the machine (dev-setup's `references/github-app.md`).
