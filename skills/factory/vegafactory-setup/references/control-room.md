...
- `dispatchers.md` is the roster of machines allowed to work a board with nobody at the keyboard: one row per machine, `| machine | group | repos | owner | caps | notes |`, the machine name being its lowercase hostname without the `.local` suffix. A machine that is not listed, or whose row is edited only in a local copy, refuses every dispatcher verb. A machine works **every repo its row lists**, in one pass, so adding a project to a box is a PR here rather than another service. The `caps` cell sets what it may do — `runs 10 · step 72h · poll 1m · retry 15m · park 3`, every field optional, found by what it says rather than which column it sits in, and a cell that means to set one and cannot be read refuses that machine. Caps belong to the machine, not to any project it works, and are re-read every pass, so a change lands on the next poll. The run cap and the retry deadlines are still per machine — two listed machines on one repository can each run their full number and can each start work the other is already holding in the hand-over window (tracked as fleet-wide leases).
