...
- `dispatchers.md` is the roster of machines allowed to work a board with nobody at the keyboard: one row per machine under a header that names the columns, `| dispatcher | group | repos | owner | caps | notes |`, the machine name being its lowercase hostname without the `.local` suffix. A cell is read by what its column is called, so a room may reorder or add columns and a notes column is never mistaken for caps; a table wider than the legacy three columns with no header refuses, because no position is known to hold them. The `caps` cell sets what the machine may do — `runs 10 · step 72h · poll 1m · retry 15m · park 3`, every field optional, and a cell that cannot be read refuses that machine by name rather than being guessed at — and is re-read every pass, so a change lands on the next poll. A machine that is not listed, or whose row is edited only in a local copy, refuses every dispatcher verb. Its run caps, its retry deadlines and the seconds between reserving an issue and the run's own claim are per machine today — two listed machines on one repository can each run their full number of steps at once and can each start work the other is already holding in that window (tracked as fleet-wide leases).
