...
A dispatcher is the third onboarding path — a machine rather than a repo or a person. `onboarding/dispatcher-box.md` provisions the always-on box under two accounts, so a CI job cannot read the dispatcher's tokens, and its row in `nodes.md` says which group's repos it serves.
