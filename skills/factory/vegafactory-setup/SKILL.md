...
1. **Ask the questionnaire.** Only what `org.md` holds and nothing a department owns: the org name, the goals in one paragraph, and what applies to everyone — the language, the date format (DD-MM-YYYY across the workflow), the "nothing ships without the operator's explicit instruction" stance, and the three statistics lines `stats:`, `stats-people:`, `stats-override:`. A knob a department decides — review, merge, the harness policy — is never asked here, because two departments reading one org answer would each need the other's to be wrong.
