...
test('worker-box onboarding describes the multi-repository worker boundary', () => {
  for (const text of [
    ...
    '`threads.json` in the same owner-only directory',
    'never prompts or transcripts',
    'a moved branch or machine starts a fresh conversation',
    ...
    'bounded, preview-only advice',
    'An unavailable housekeeping row means repository facts were incomplete or timed out',
    'A person checks the bare preview and decides whether to run `prune --write`',
