...
Every skill this repo ships is scanned by [NVIDIA SkillSpector](https://github.com/NVIDIA/skillspector) through the `skill-scan` skill's `scripts/skill-scan.mjs`, in the merge queue on the built bundle, before any change reaches main. The guard blocks on any unsuppressed **HIGH or CRITICAL** finding and ignores the aggregate risk score: a skills repo documents the very mechanics the scanner matches on, so the score reflects our subject matter more than our risk.
...
- **Fingerprints are platform-bound.** SkillSpector's content fingerprints differ between macOS and Linux, and the merge queue scans on Linux: take refreshed fingerprints from the `skill-scan-fingerprints` artifact a blocked merge-queue run uploads, never from a local Mac run.
...
- **`P2` fires on every HTML comment**, because a hidden instruction is a genuine injection vector. This repo uses HTML comments as machine-readable markers — `vsk:v1`, `vsk-dev:start`, `<!-- mirrored -->` — so the finding is the documentation of a mechanism, not an instance of one. Scope the rule to the file, never to the id alone.
