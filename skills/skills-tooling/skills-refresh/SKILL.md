...
| Need | Read |
|---|---|
| the tools, their facts-file sections, the fact-line format, the window | [watchlist](references/watchlist.md) |
...
Read-only. It parses [the watchlist](references/watchlist.md) and every facts-file section it names, and returns each tool with its fact count, how many are past the 60-day window, and the oldest age — plus a `due` array naming each stale line by tool, file, line number, capability and link. **Exit 0 nothing due · 1 something is due · 2 a file or a line could not be read.** Exit 2 is a malformed fact line or a missing file, and it is fixed before the sweep runs, because a line the scanner cannot read is a fact nobody is checking.
...
## 2 — Read it, one subagent per tool

**One tool, one subagent.** Never one agent for several: the vendors' pages have nothing to do with each other, and an agent reading seven changelogs remembers the first one badly. The watchlist is built for this — one row per tool, each naming that tool's own facts-file section and that vendor's own pages, and no two rows resolving to the same section. Give each subagent one row: its section's lines and its pages, nothing else. Ask for exactly this back:
