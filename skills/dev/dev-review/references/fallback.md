...
- The command checks first that the other tool really cannot review: it is not installed, it does not run, or a real review run failed on its credentials. A status command is not proof — Codex prints "Logged in" on a revoked token — so the way to prove a signed-out tool is to run the review once and let it fail: the failure is recorded against that head and the fallback then cites it ("codex failed to authenticate during round 1"). While the tool is installed and nothing has failed, `--record` refuses. The operator can allow one anyway, for a given head, in a line of their own on the issue: `accept same-tool review @ <sha7>`.
