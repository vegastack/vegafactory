...
- The command checks first that the other tool really cannot review: it is not installed, it does not run, or a real review run failed on its credentials. A status command is not proof — Codex prints "Logged in" on a revoked token — so the way to prove a signed-out tool is to run the review once and let it fail on its own credential error (Codex's `token_revoked` / `refresh_token_invalidated`, Claude Code's login message, on its own stderr — a hook or a model saying "401" is not that). The failure is recorded against that cycle, round, head and artifacts, the fallback cites it once ("codex failed to authenticate during cycle 1 round 1"), and it is deleted as soon as it is used or the tool reviews successfully. While the tool is installed and nothing has failed, `--record` refuses. The operator can allow one anyway, for a given head, in a line of their own on the issue: `accept same-tool review @ <sha7>`.
