import { describe, expect, test } from 'bun:test'

// Acceptance: a review comment in this skill's format is typed and read by the CLI's
// issue cache (first-line marker). Cross-package import is test-only — tests never ship.
import { commentType } from '../../../../packages/cli/src/issue-cache.ts'
import { markerKeys } from '../../../../packages/cli/src/issue.ts'

describe('review comment ↔ issue cache contract', () => {
  const reviewComment = `<!-- vsk:v1 type=review round=2 sha=abc1234 agent=codex verdict=clean -->
## Review — round 2 @ abc1234

**Verdict: clean** — spec: 0 · standards: 0 · security: n/a (no surface)

## Review — round 1 @ def5678

**Verdict: needs-fixes** — spec: 1 must-fix
`
  test('the cache reads the top marker: type, verdict, sha, agent', () => {
    expect(commentType(reviewComment)).toBe('review')
    expect(markerKeys(reviewComment)).toEqual({ type: 'review', round: '2', sha: 'abc1234', agent: 'codex', verdict: 'clean' })
  })
  test('appended prior rounds carry no marker, so the first marker stays current', () => {
    const afterTop = reviewComment.split('\n').slice(1).join('\n')
    expect(commentType(afterTop)).toBe('human')
  })
})
