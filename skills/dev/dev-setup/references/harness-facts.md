...
Which model and which reasoning effort a stage runs at is dev.md's `harness-policy:` knob, written `<stage> <agent> default|<model id> <effort>`. The literal `default` in the model position means "no model flag — the tool's own default", which is what a subscription account should use: a pinned id the account cannot serve fails the whole run (seen 18-09-2026: `codex exec -c model=gpt-5.6` on a ChatGPT account answers "The 'gpt-5.6' model is not supported when using Codex with a ChatGPT account"). The effort flag is always passed. These are the flags each value turns into.
...
| Codex | `codex exec -m <model>`, or `-c model=<id>` as a config override; omitted entirely for `default` <!-- source: CODEX-CONFIG --> | `-c model_reasoning_effort=<level>` — the levels in the shipped 0.153.4 binary's own enum are minimal, low, medium, high, xhigh, max and ultra (read 18-09-2026; the docs demonstrate `"high"` and enumerate nothing) <!-- source: CODEX-CONFIG --> | `agents.max_concurrent_threads_per_session` in config.toml caps concurrently open spawned-agent threads, excluding the primary; unset means Codex picks the default <!-- source: CODEX-AGENTS-MULTI --> <!-- source: CODEX-CONFIG --> |
...
- **Neither tool's status command proves it can run.** `codex login status` prints "Logged in using ChatGPT" while the session's refresh token is revoked and real runs fail with `refresh_token_invalidated` / `token_revoked` (recorded 03-09-2026, unchanged 18-09-2026); `claude auth status` prints `"loggedIn": true` from the same kind of cached state. So availability is judged from `--version` (is it installed at all) plus a review run that actually failed on credentials. <!-- source: CODEX-EXEC --> <!-- source: CC-CLI -->
