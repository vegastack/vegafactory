...
`session-start.mjs`, `stop-heartbeat.mjs` and `session-end.mjs` take `--harness claude|codex`, run with a one-second timeout, and currently do nothing: the session context, heartbeat and statistics they fed are being rebuilt as one `vegafactory hook` command (#216).
