...
The runtime emits the existing recovery arm `remoteEffectCoverage:{kind:"unmanaged-possible",reasonCode:"hook-configuration-only"}` in launch/tick evidence. No configuration inspection or local denial promotes it to qualified mediation; #137/#158 own receipt validation. Parallel dispatcher execution currently refuses until its child gateway reuses the same checked executor for every prepared child; raw command descriptions are not execution evidence. Refusal before the OS acknowledges spawn neither records a launch nor consumes a corrections reaction.
