...
The ship guard's policy is a fixed always-ask list — a push to the default branch, `gh pr merge` and a merge through `gh api`, `git tag` and pushing tags, publish commands, `gh release` changes, a force push, `git reset --hard`, deleting a branch (local or remote), `git worktree remove`, `--no-verify`, any `gh api` call that is not a plain GET (a method other than GET, a body from `-f`/`-F`/`--field`/`--raw-field`/`--input`) and any GraphQL mutation, raw `gh issue comment` and `gh pr comment`, `vegafactory issue ack` unless it is a brief or plan ack citing the operator's comment (`--source comment:<id>`), and `vegafactory issue claim --take-back-by` — plus every command named in backticks on an `ask:` line of the `## Ship` section of `.vegastack/dev.md`, read from the committed default branch (`git show origin/<default>:.vegastack/dev.md`), so a task cannot loosen it by editing its own copy. One exception: `gh pr merge <pr>` passes when the PR's branch names issue n (`<type>/<n>-…`) and n has a valid "ship it" ack after its latest evidence comment; `--admin` or `--repo` always ask. The guard reads the command as a shell would — quotes, escapes, `;` `&&` `||` `|` `&`, subshells and dollar-paren command substitution — resolves wrappers (`sudo`, `env`, `nice`, `time`, `timeout`, `xargs`, `sh -c`, a path or an escape on the head) and git and gh global options, then matches the resolved argv: a push's destination in every refspec spelling (`HEAD:main`, `refs/heads/main`, `main:main`, `+main`), force and delete flags in any position, `gh api` on a merge URL, and a probe of text handed to another interpreter. A guarded command it cannot read — a command name or a guarded command's argument built by shell expansion (`$VAR`, `${…}`, dollar-paren, backticks, `$'…'`), arguments fed by `xargs` or `find -exec`, an unknown git alias, a push with no readable destination, no default branch on `origin`, a shell tool whose payload has no readable command (Claude's `Bash` and Codex's `shell` send `tool_input.command`, Codex's `exec_command` sends `tool_input.cmd`, as a string or an argv array), an unreadable payload — asks, never allows. An oversized payload from a file tool still gets the ownership check; only known read-only tools pass without one. `vegafactory issue comment`, `edit-comment` and `body` refuse text whose top marker is `type=ack`, `type=claim` or `type=release`: only the dedicated verbs write those.
