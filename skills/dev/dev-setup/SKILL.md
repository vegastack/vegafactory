...
| org defaults (control room) | `vegafactory sync --json` to refresh the copy, then `vegafactory sync profile --json` for the answer: it returns the resolved `values`, the `sources` naming which layer each came from, the org's `locked` lines, and `blocks`. Read the profile from that command and never by opening files in the copy — the command is what checks the copy is the one the record describes, and a file read around it is a file read around every one of those checks. On a repo with no `control-room:` knob yet, the first run here, pass the org from the repo lookup above to `sync` (`--org`), because `sync` otherwise reads the knob this skill has not written. Skills read the copy, never the network, so an outage degrades to a stale answer instead of failing; a control room that answers a knob removes that question from Round B; a non-empty `blocks`, or no room at all, is a plain finding and Round B asks; layout and precedence are the `vegafactory-setup` skill's |
...
| harnesses present | `command -v claude codex` and each present one's `--version` — the names and versions become the `harnesses:` knob line, and the AGENTS.md block, the CLAUDE.md import and the hook offer target only harnesses that exist. Any harness the drafted `harness-policy:` names but the box lacks is absent → record it in `## Environments` with the capability it gates (no Codex → cross-tool review is off and reviews fall back to a labelled same-tool self-review), naming no install command, because the vendor's own docs own that |
...
| VegaFactory CLI | nothing to detect — session hooks and dispatchers read `vegafactory-update:`; confirm the drafted value instead (`auto` keeps the global CLI current, `notify` only reports, `off` makes no registry check) |
...
Ask with your harness's question tool — AskUserQuestion in Claude Code, `request_user_input` in Codex where the mode allows it ([harness-facts](references/harness-facts.md)). When none is available (headless run, gated mode), write the defaults, mark every unconfirmed knob `# TODO confirm`, and say so, because a wrong invented preference costs more than a TODO. The route the other dev skills take when no tool is available is [ask-route](references/ask-route.md); dev-setup can run before any issue exists, so its own fallback stays the documented defaults above.
...
1. Nothing to choose for review: `vegafactory review` always runs the other tool. Detection found only one harness on the box → say plainly that cross-tool review is off until the other one is installed and that reviews fall back to a labelled same-tool self-review; record the gap in `## Environments`. The `harness-policy:` line is still drafted in full, so the stage set does not change with the box
2. Proof for UI work: **playwright screenshots** · none
3. Tests: **required for every change** · required for logic changes only
4. Who may be assigned issues here (`operators:` knob): **just you** (the detected login) · a csv of logins — every human who can receive a `waiting-on-operator` or `ready-to-ship` issue
...
- Confirm the drafted `vegafactory-update:` value: `auto` checks attended session starts and updates in the background, and updates an idle dispatcher between passes; `notify` reports the available version without changing the machine; `off` makes no registry check
...
- Harnesses detected → confirm the drafted `harness-policy:` line — `<stage> <agent> default|<model id> <effort>`, six fixed stages, `default` in the model position (the recommended value: the tool's own model, nothing to go stale) and the effort the operator wants; the flags each value turns into are in [harness-facts](references/harness-facts.md). A pinned model id the account cannot use fails loudly on the first run, and the fix is this knob line, never a skill edit
- Hooks → offer them as one set, on one yes, per [harness-facts](references/harness-facts.md): every event runs `vegafactory hook <event> --harness <h>` — the ship guard (asks before the always-ask commands and the backticked commands on the `## Ship` section's `ask:` lines), session context, the claim heartbeat and a WIP commit each turn. A deploy that needs the operator's word goes on an `ask:` line with its command in backticks. Merge into both config layers, preserving user hooks and removing this package's old `.vegastack/hooks/` entries. Codex needs the project's `.codex/` layer trusted per worktree — say so before wiring.
...
| labels | `gh label create <name> --color <hex> --description "<text>"` for validated semantic state names plus existing scope/modifier labels, skipping ones that exist; creation colors ([conventions](references/conventions.md) holds meanings): state `waiting-on-operator` FBCA04 · `planning` E36209 · `queued` 0E8A16 · `in-progress` 1D76DB · `ready-to-ship` 5319E7; flags `risky` B60205 · `epic` 24292E (only when the org has no native Epic issue type); sizes `small` C2E0C6 · `medium` 76C7C0 · `large` 2A9D8F · `research` C5DEF5 |
...
| project `.gitignore` | add `.vegastack/.tmp/` and `.vegastack/.worktrees/`, each when absent — branches are checked out in one and drafts, the issue cache and the lessons queue are written in the other ([conventions](references/conventions.md)); without both lines the turn checkpoint commits and pushes them |
| guard workflows / hook wiring | only the ones the user said yes to in Round C; the hook wiring from [harness-facts](references/harness-facts.md) goes to `.claude/settings.json` (Claude Code) and `<repo>/.codex/hooks.json` (Codex), merged never replaced |
