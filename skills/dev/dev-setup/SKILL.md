...
| guard workflows / hook wiring | only the ones the user said yes to in Round C; a workflow or dev.md section the control room's `templates/` overrides is taken from there and this skill's own asset is the fallback, because the org's template is a deliberate default and these are not; the hook wiring from [harness-facts](references/harness-facts.md) goes to `.claude/settings.json` (Claude Code) and `<repo>/.codex/hooks.json` (Codex), merged never replaced |
