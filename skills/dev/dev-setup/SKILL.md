...
| org defaults (control room) | `vegafactory sync --json` first — on a repo with no `control-room:` knob yet, the first run here, `vegafactory sync --org <org> --json` with the org from the repo lookup above, because `sync` otherwise reads the knob this skill has not written — then read the local copy it names: skills read the copy, never the network, so an outage degrades to "last synced <time>" instead of failing; a control room that answers a knob removes that question from Round B; a room that does not exist or cannot be read is a plain finding, and Round B asks; layout and precedence are the `vegafactory-setup` skill's |
