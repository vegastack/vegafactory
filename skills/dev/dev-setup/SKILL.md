...
| VegaFactory CLI | nothing to detect — session hooks and workers read `vegafactory-update:`; confirm the drafted value instead (`auto` updates the global CLI in the background, `notify` only reports, `off` makes no registry check) |
