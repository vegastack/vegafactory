...
5. Who may be assigned issues here (`operators:` knob): **just you** (the detected login) · a csv of logins — every human who can receive a `waiting-on-operator` or `ready-to-ship` issue
...
- Hooks package → offer the ship guard per [harness-facts](references/harness-facts.md), on its own yes: confirm policy, then compile the guard policy; `vegafactory guard sync` refreshes later edits. Merge into both config layers while preserving user hooks. The session hooks (context, heartbeat, decision reminder) are not offered until they are rebuilt (#216). Trust Codex's project `.codex/` per worktree.
