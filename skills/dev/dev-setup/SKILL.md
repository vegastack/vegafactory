...
- Hooks package → offer two per [harness-facts](references/harness-facts.md), each on its own yes: the ship-guard confirmation (on that yes, confirm policy, then compile the guard policy; `vegafactory guard sync` refreshes later edits) and the decision hook (directional choices stay session proposals). Deduplicate both config layers while preserving user hooks; replace/report only this package's legacy decision-nudge.sh in place. The SessionStart, Stop and SessionEnd hooks are not offered until they are rebuilt (#216). Trust Codex's project `.codex/` per worktree.
