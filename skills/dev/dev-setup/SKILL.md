...
- Hooks → offer them as one set, on one yes, per [harness-facts](references/harness-facts.md): every event runs `vegafactory hook <event> --harness <h>` — the ship guard (asks before the always-ask commands and the backticked commands on the `## Ship` section's `ask:` lines), session context, the claim heartbeat, a WIP commit each turn, and one request per working session for the general lessons it taught, which a later session proposes as dev.md lines for the operator's yes. A deploy that needs the operator's word goes on an `ask:` line with its command in backticks. Merge into both config layers, preserving user hooks and removing this package's old `.vegastack/hooks/` entries. Codex needs the project's `.codex/` layer trusted per worktree — say so before wiring.
