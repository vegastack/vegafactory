...
Optional. Declare it only when the sibling sub-issues below could run at the same time; its absence means they run in plan order, which is the safe default. A worker runs the declared groups at the same time; undeclared siblings run one at a time.
...
- `node <path-to-this-skill>/scripts/plan-lint.mjs --file <plan.md> --groups --json` prints the validated groups as JSON — the one parser for this grammar, and the one the worker reads to decide which sibling sub-issues may run at the same time. Blocked plans print no groups.
