...
- The declared set is also the **scope check**: a sub-issue whose diff touches a path outside its own set is not merged until the operator says so.
- `node <path-to-this-skill>/scripts/plan-lint.mjs --file <plan.md> --groups --json` prints the validated groups as JSON — the one parser for this grammar, read by the dispatcher to decide which sibling sub-issues may run at the same time (at most three; generated files, migrations, lockfiles and `package.json` never run in parallel).
