...
- Resume unfinished work only within its original ack, using [ledger-and-resume](references/ledger-and-resume.md): the status comment and `git log` on the issue branch recover the position; the brief and plan confirm the scope still holds. Restore a missing worktree with `vegafactory worktree restore <n>`. Another session's claim is taken back only on the operator's word: `vegafactory issue claim <n> --harness <h> --model <id> --take-back-by <their login>`.
...
- **Tasks inside one issue run in order, and so do sibling sub-issues.** A plan's independent groups record which siblings *may* run at the same time — non-overlapping file sets, each its own issue, branch and worktree — but running them is the dispatcher's job (#218). Until it lands, you work them one at a time and never launch a second session yourself.
