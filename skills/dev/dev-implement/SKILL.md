...
One session owns one issue — one claim at a time, because dev-status reads each ledger as one session's heartbeat. Assign yourself and remove any other assignee, move the issue with `vegafactory issue label <n> --state in-progress`, and cut the branch and its worktree with `node <path-to-this-skill>/scripts/worktree.mjs create --issue <n> --slug <slug> --type <type> --write --json` (sub-issue of an epic: add `--parent <parent-branch>`; the full matrix is [worktrees](references/worktrees.md)). Then post the ledger comment (`vegafactory issue comment`) as your first write, recording the worktree path, because its edit time is the claim's only liveness signal. Record each task's base sha before starting it. A hand-back to `ready-to-ship` assigns the issue's operator and drops the runner, so the operator's notification is GitHub's own.
...
- **Tasks inside one issue run in order.** Sibling sub-issues of an epic also run one at a time for now; the plan's independent groups record which could run in parallel once the dispatcher (#218) does so.
