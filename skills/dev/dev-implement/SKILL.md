...
- **Tasks inside one issue run in order, and so do sibling sub-issues.** A plan's independent groups record which siblings *may* run at once; running them is the dispatcher's job (#218). Until it lands, work them one at a time.
