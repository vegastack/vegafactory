...
- **Claude Code hooks:** the environment variable named `CLAUDE_PROJECT_DIR` stays at the launch root, while the hook input's `cwd` follows the worktree — hooks that need to know where the work is read `cwd`.
