...
- Record one schema-v2 approval event per conventions: `scope=brief`, or `scope=brief+plan` for brief plus plan comment. Use dev-implement’s `artifactRef` on refreshed canonical artifacts; include the actual operator, exact source quotation, unique event ID and bindings. Evaluate the complete history against current `operators:` before changing state. A marker or agent author alone grants nothing; preserve legacy records and request exact scoped reconfirmation instead of converting them.
