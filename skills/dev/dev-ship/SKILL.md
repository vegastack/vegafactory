...
Observed excuse: "opening a PR is preparation, not shipping." It waits for the operator's word all the same — finished work parks on the pushed branch, and a draft PR is still a PR nobody asked for.

## The merge — one word, the whole sequence

On "ship it", run it through: PR → merge queue → merge → the issue's worktree cleanup → the Ship runbook. Report each step as it lands; stop only on a failure or a fact the operator alone holds. Cleanup comes **before** the runbook, because the runbook stops at its release steps and a cleanup queued behind a stop never happens.
