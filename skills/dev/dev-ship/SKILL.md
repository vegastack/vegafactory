...
Act on the operator's word only. Per issue the operator gives two words: an ack on the brief or plan (it authorized building) and **"ship it"**, which carries that issue's landing — PR, merge queue, merge, cleanup. Two words per issue, not one per step. Not the release, though: a generated release PR belongs to no issue, so its merge and the tag push still ask ([runbook](references/runbook.md)).
...
- Verify the changelog state matches the evidence comment's `**Changelog:**` line: a behavior-changing branch carries its entry per dev.md's `changelog:` knob and, with `chronicle: on`, its chronicle entry, while `none` with a reason that holds up (docs-only, test-only) is fine. An unexplained miss → corrections loop, not a PR.
...
- A direct chat change (dev-implement's no-issue path) ships on the same words: the chat request stands in for the recorded ack, and the PR body carries the evidence instead of linking an issue comment.
- Corrections left on the PR itself flow through the same loop on the same branch — the PR updates with the push; nothing is recreated.

## The merge — one word, the whole sequence

On "ship it", run it through: PR → merge queue → merge → the issue's worktree cleanup → the Ship runbook. Report each step as it lands; stop only on a failure or a fact the operator alone holds. Cleanup comes **before** the runbook, because a cleanup queued behind the runbook's release stop never happens.

- Run `vegafactory ship check <n>` (`--json` for the reasons as data). It passes only when the issue passes `issue check --for ship` (a valid "ship it" after the latest evidence), the branch is clean and pushed, its PR is open on that commit with every check green, and that commit carries a clean cross-tool review of the current brief and plan, whose base is already in the default branch — or, once the third round of a cycle is spent, the operator's own comment accepting the open findings on a line of its own (`accept review round 3 @ <sha7>`), which only they may write. A review recorded as `mode=same-tool` is the documented fallback and still counts — it is on the record as one. Exit 2 stops: report its reasons and route them — code problems to implement's corrections loop, a missing word to the operator.
...
- Pending `Decision:` lines exist (issue comments, or the evidence comment's `**Decision:**` line) → name them before merging — "merging will record: …" — and act on the operator's confirmation, because the register is append-only and an inferred line cannot be taken back. On the word, append each to the register dev.md names in conventions' Operator identity format.
...
- Then, before the runbook, remove the merged branch's worktree — `vegafactory worktree remove <n> --json` — which takes the **directory only**. Deleting the local or remote branch is its own operator word.
- A bot PR (Renovate, Dependabot) has no issue or evidence comment and merging it is still shipping: green checks qualify it, only the operator's explicit word — per PR or per named batch — merges it, and majors and advisories get named first ([runbook](references/runbook.md)).

## After the merge — the Ship runbook

Merge and cleanup are not the end when dev.md has a `## Ship` section: follow its steps in order — `auto:` lines you just do under the same "ship it", `ask:` lines wait for the operator's own word (the guard asks for any command they name in backticks, read from the default branch's dev.md), `guard:` lines are deterministic checks you run locally at their position (their CI copies are the backstop). With `release: per-merge`, the runbook is part of shipping the issue; with `release: on-request`, it runs only when the operator says "release" (covering everything merged since the last one). Report each step's outcome; a failing step — guard included — stops the sequence and goes to the operator. Execution detail, the release exception, release batching, bot PRs, and rollback: [runbook](references/runbook.md).
