...
- Run `vegafactory ship check <n>` (`--json` for the reasons as data). It passes only when the issue passes `issue check --for ship` (a valid "ship it" after the latest evidence), the branch is clean and pushed, its PR is open on that commit with every check green, and that commit carries a clean cross-tool review of the current brief and plan, whose base is already in the default branch — or, once the third round is spent, the operator's own comment accepting the open findings on a line of its own (`accept review round 3 @ <sha7>`), which only they may write. Exit 2 stops: report its reasons and route them — code problems to implement's corrections loop, a missing word to the operator.
