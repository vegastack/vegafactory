...
- Run `vegafactory ship check <n>` (`--json` for the reasons as data). It passes only when the issue passes `issue check --for ship` (a valid "ship it" after the latest evidence), the branch is clean and pushed, its PR is open on that commit with every check green, and that commit carries a clean cross-tool review — or, for findings the review left open, the operator's own comment accepting them by round and head (`accept review round <n> @ <sha7>`), which only they may write. Exit 2 stops: report its reasons and route them — code problems to implement's corrections loop, a missing word to the operator.
