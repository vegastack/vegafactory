...
The tag is not part of that exception: `vegafactory ship release <n>` re-reads issue n's own recorded "ship it" — against the current evidence, so evidence posted or edited after the word spends it exactly as at merge — checks that the version and its changelog entry agree, requires the default branch checked out, clean and level with origin, and then creates and pushes `v<version>` itself. Issue n belongs to the repository this checkout pushes to; there is no `--repo` to point it elsewhere. The guard allows that one verb, spelled plainly with the issue number, while raw `git tag` and tag pushes keep asking. It never publishes — the tag-triggered workflow does — and it refuses (exit 2) rather than tagging anything it cannot prove, so a refusal leaves the repository as it was.
...
- **Worktrees are pruned, not swept.** Bare `vegafactory worktree prune` previews; only `prune --write` acts. It considers merged and closed checkouts immediately and retained-idle checkouts after `worktree-retention:`, never creates a remote branch, and keeps anything dirty, staged, unpushed, locked, unsafe, or uniquely detached. `--force` on `remove` and branch deletion always take the operator's word.
